...
**Refinement (post-1.10.0): first visit follows the browser language.** The original "always start in English" default was a deliberate worst-case-safe choice, but it meant e.g. a German visitor saw English until they found the setting. `store.js` now, **only when there is no persisted `store_locale`**, matches `navigator.languages` (preference-ordered) against the shipped locales — exact tag, then primary subtag (`de-DE`→`de`, `ru`→`ru-RU`, `en-GB`→`en-US`) — and starts there, still falling back to `en-US` when nothing matches. This is safe against the flicker/SSR-mismatch concern that blocked it earlier: it runs **before** `setCurrentLocale` and the first paint, and the only language-dependent first-paint surface (the logged-out landing UI, `NotLoggedInHome`) is `HiddenFromSSR`, so the server never commits to a language the client then has to correct. The dropdowns reflect it automatically because they bind `defaultValue={$locale}` and the locale is already set before they render. The detected value is not eagerly persisted — it becomes the stored preference on the next `save()` (LocalStorageStore flushes dirty keys on lifecycle 'passive'/tab-blur), so a first-visit detection still behaves as "detect once, then stick", and an explicit dropdown choice overrides it immediately.
