...
**Refinement (v1.11.1) — fail-fast first attempt.** The first cold attempt now uses the normal read timeout (`SLOW_READ_TIMEOUT_FIRST` = `DEFAULT_TIMEOUT`, 20 s) instead of the full 40 s; only the retry keeps the 40 s headroom. A genuinely *hanging* cold list previously blocked ~80 s (both attempts at 40 s) before the graceful fallback; now it's ~60 s, and the common warm-up case is unchanged or faster (the retry starts sooner against a now-warming backend). See §22.
...
- **Turn it non-exclusive / remove a member / delete the list** → mark home **stale** so the next fetch brings them back, server-filtered (no retroactive cache rebuild — §20 [v1.9.2] model).
