...
2. **One VAPID key per subscription.** A subscription is created with a single `applicationServerKey` (the server's VAPID public key). The push *service* (Mozilla autopush / Google FCM / Apple) validates **every** incoming push's VAPID JWT signature against **that** key and **drops** messages signed by any other key — *before they ever reach the service worker*. Each Mastodon/GoToSocial instance has its **own** VAPID keypair (`server_key`), generated at instance setup; they differ between servers.

**What the two constraints imply together.** The single browser subscription can be bound to exactly **one** instance's VAPID key at any moment. Pushes from that instance validate and arrive; pushes from every **other** logged-in instance are **rejected by the push service** and never arrive. If a second account re-subscribes (with its own key), it **re-keys** the shared subscription and **breaks the first**. The accounts effectively **fight over the one mailbox**. Net effect: **OS push reliably works for at most one account per device at a time**, and *which* account is simply whichever one last (re-)keyed the subscription — non-deterministic from the user's point of view.

**A third, related problem: routing.** Even when a push *does* arrive, Mastodon's payload doesn't say which account it's for (`mastodon#22183`). The SW can only enrich a notification (fetch the full object, attach per-type deep-links and reblog/favourite actions) when there is exactly **one** known instance (`getKnownInstances().length === 1`); with several accounts it falls back to the server-provided title/body. So even the "one working account" case loses fidelity the moment a second account exists locally.
...
**Crucial caveat: these mitigations make the *lifecycle* correct; they do NOT make multi-account push work.** No client-only fix can overcome constraint #2 (different VAPID keys). The guards remove the friendly-fire and the logout leak, but with two accounts on different servers you still get reliable OS push for only one of them.

**The three architectural options — for when this is revisited:**

| Option | What it is | Pros | Cons |
|---|---|---|---|
| **C — Honest single-account push** *(recommended target)* | Make the platform truth explicit: only **one** account can have OS push per device. Enabling push on B disables it on A (re-keys the subscription to B), with a clear UI message ("Push is limited to one account per device — moved to @…"). | Tiny code; no infrastructure; **deterministic** (the *user* picks the push account instead of a non-deterministic race); removes the `otherInstancesWantPush` complexity (by construction there's never a second push account); strictly **better than today** (today only one account works anyway, just unpredictably). | A multi-account user is notified about only one account while the app is closed; the others must be checked in-app. (But that is already the de-facto reality.) |
...
| **D — Per-instance map + lifecycle guards** *(current state, v1.8.5)* | The shared subscription with the `otherInstancesWantPush` teardown guard + logout backend-unsubscribe. | Correct lifecycle, no regressions, self-heal; low-risk incremental fix. | Polishes a model that **cannot** carry >1 account (VAPID limit unaddressed); the per-instance map stays a partial illusion. |
...
**Recommended direction:** keep **D** now (the v1.8.5 guards — correct and risk-free); move toward **C** ("honest single-account push") as the clean target model for a client-only PWA. Pursue **B** (relay) only if true parallel multi-account push becomes a hard requirement — as its own project, bundled with the self-hosted-services backlog. **The trap to avoid:** continuing to extend the per-instance illusion (more maps, more guards) as though it were real parallelism — constraint #2 means it can never be.
...
**Tradeoff / explicit non-fix:** the guards fix the lifecycle but **cannot** make multi-account push actually work — no client-only code overcomes the differing-VAPID-key constraint, so with two accounts on different servers OS push still reliably reaches only one. The genuinely robust options are **C** (make the single-account limit explicit and user-chosen — recommended for a client PWA; *simpler* than the current model, not more complex) or **B** (a self-hosted push relay with one VAPID key, which also solves push→account routing — the only path to true parallel push). Current state (**D**) is the correct incremental step; the trap to avoid is extending the per-instance illusion further as if it were real parallelism.
...
| **1.8.5** | 2026-06-12 | Multi-account Web Push lifecycle fixes: the shared per-origin browser push subscription is no longer torn down when one account disables push or its push service gives up (`otherInstancesWantPush` guard), and logout now deletes the account's subscription on its **backend** so its server stops pushing to the shared endpoint. Documented the single-subscription / single-VAPID platform reality and the three architectural options (honest single-account vs. self-hosted push relay vs. current guards). Defensive `currentTimeline` null-guard in `setupTimeline` |
