...
- **Action-error logging sweep:** factor the per-action `console.error` for transient network failures into a shared `logActionError` helper (warn on network noise, error otherwise), so all `_actions/*` match the follow/block/remove behaviour. Also a few hardcoded user-facing strings (e.g. the remote-reaction refusal toast in `react.js`) want intl keys.
