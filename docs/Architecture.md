...
- **IndexedDB / database lifecycle** (`_database/*` beyond the keyval store): per-instance DB migrations, connection close on freeze, pagination. Partially exercised by C+'s new keyval consumer; the broader per-instance DB layer is unreviewed.
