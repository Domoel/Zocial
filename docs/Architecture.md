...
**Design choices:**
- **Unfollow = follow-gated timelines (home + lists).** Inclusion in home *and* in lists is gated by following (a list can only contain accounts you follow; unfollowing removes them from your lists), so both are purged. `removeAccountFromFollowGatedTimelines` builds the set `{home} ∪ in-memory list/* ∪ list/<id> for each id in instanceLists` (the list set covers *unloaded* lists' IDB pointers too) and purges each **per-timeline** (keyrange) so local/federated/tag/account are untouched. **Block = everywhere:** `removeAccountFromAllTimelines` enumerates the instance's cached timelines via `getAllTimelineData` and filters each; the DB purge passes `timeline = null` to iterate the whole `status_timelines` store.
