...
The major neuralgic subsystems have now each had a focused pass (push, streaming, compose, virtual-list, DB lifecycle, timeline read/hydration, auth/OAuth, service worker, word filters). Remaining lower-criticality areas, only as future opportunities:

- **API/ajax endpoint modules** (`_api/*`) and **status-action plumbing** (`_actions/react.js`, `favorite.js`, `deleteStatuses.js`, …): mostly thin wrappers, stable.
- **Compose autosuggest** (`autosuggest*` actions/observers) and **emoji/search**: medium complexity, not yet a focused pass.
- Carry-over notes from earlier passes worth folding in if revisited: `getNotification` by-reference caching (timeline read pass), `deleteDatabase` `onblocked` never settling (DB lifecycle pass).
