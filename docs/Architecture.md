...
- **Turn a list exclusive / add an account to an exclusive list** → purge the affected member(s) from home (in-memory `home` summaries + IDB `home\0…` pointers via `deleteTimelineItemsForAccount(…, { homeOnly })`). Toggling on fetches the members first (`getListAccounts`, `limit=0` = all, no pagination).
