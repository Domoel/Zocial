...
- **Auth / instance management & OAuth** (`_actions/addInstance.js`, `instances.js`, `_api/oauth.js`, `_utils/ajax.js`): OAuth redirect/token exchange, access-token storage, instance switching, full logout teardown. Security-sensitive and only grazed so far (via push logout) — the top remaining unreviewed area.
