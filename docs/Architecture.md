...
**2. Two distinct hints in Settings** (`DeviceNotificationSettings.html`) — split by kind, because they are different things:
  - **Static constraint → a permanent `Tooltip`** (the focus-ring-style hover/tap component) on the master toggle label, shown whenever more than one account is logged in, *independent of push state* so it never disappears or clutters the clean enabled state. Visible trigger *"(one account per device)"*; the popup elaborates *why* (one Web Push subscription per device). This is the durable, discoverable explanation of the limit.
