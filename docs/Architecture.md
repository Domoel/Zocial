...
**Home timeline on delete.** Deleting an **exclusive** list ends the server-side home exclusion for its members. The home cache is union-only and won't pull them back on its own, so `deleteListById` marks the home timeline **stale** → the next `setupTimeline` (poll/navigate) re-fetches and the members reappear. Consistent with the §20 [v1.9.2] model (server-driven; a refresh surfaces them, no retroactive cache rebuild). Turning exclusive **on** deliberately does **not** purge already-cached home posts (minor, self-heals — confirmed acceptable).
