...
### [v1.9.1] Remove from followers (`remove_from_followers`) — confirmed + capability-gated

**Decision:** Support Mastodon's `POST /api/v1/accounts/:id/remove_from_followers` as a `removeFollower` item in the account "⋯" options menu, **gated on `relationship.followed_by` AND backend support** (`$supportsRemoveFromFollowers`). The `followed_by` gate surfaces it exactly where it's useful — every row of the Followers list (where the ⋯ menu now lives) and on the profile of anyone who follows you — and auto-hides it on foreign followers lists where the person doesn't follow you.
...
**Backend handling — proactive hide, not show-then-toast:** the endpoint isn't universal (verified June 2026: Mastodon `401`/exists, GoToSocial `404`/absent). Showing a dead item that errors on click is poor UX, so the item is **hidden unless the backend is known to support it.** Detection uses the **authoritative nodeInfo `software.name`** (`supportsRemoveFromFollowers` computed): `mastodon` (incl. forks like glitch-soc that keep the name) and `hometown` → shown; everything else (GoToSocial, Pleroma/Akkoma, Friendica, unknown, or nodeInfo not yet loaded) → hidden. This is chosen over version-sniffing because fork version strings are unreliable and a future GtS could share Mastodon's numeric range. As **defence-in-depth**, the action still degrades on **404/501** to a `removeFromFollowersNotSupported` toast (covers a detection gap). On success `updateLocalRelationship` sets `followed_by:false` (the item self-hides) and `emit('refreshAccountsList')` drops the row from the Followers list. Icon `#fa-times` (distinct from Unfollow's `#fa-user-times` and Block's `#fa-ban`).

**Files:** `_api/follow.js` (`removeFromFollowers`), `_actions/follow.js` (`removeAccountFromFollowers`, 404/501 graceful), `_store/computations/instanceComputations.js` (`supportsRemoveFromFollowers`), `_components/dialog/components/AccountProfileOptionsDialog.html` (`followedBy` + `$supportsRemoveFromFollowers` gate, item, confirm handler), `intl/*` (`removeFollowerAccount`/`removeFollowerTitle`/`removeFollowerText`/`removeFollowerConfirm`/`removedFollower`/`removeFromFollowersNotSupported`, all 5 languages).

**⚠️ Backlog — the allowlist does NOT auto-adapt.** The gate is a *static* software-name allowlist, so a backend that adds `remove_from_followers` later (notably **GoToSocial** — absent at 0.21.2 / June 2026) will keep the item hidden until the allowlist is updated; the Fediverse advertises no capability flag for this endpoint, so there's no click-free way to auto-discover it. When a backend ships it, add it (ideally refactor `supportsRemoveFromFollowers` into a version-aware `{ software: minVersion }` map compared against `nodeInfo.software.version`, so it then auto-enables for all instances ≥ that version). Re-probe with `POST …/remove_from_followers` → `401/403` = supported, `404` = not. Tracked in memory `project_remove_from_followers_capability_backlog`.
...
| **1.9.1** | 2026-06-13 | "Remove from followers" (Mastodon `remove_from_followers`) in the account "⋯" menu — gated on `followed_by`, with a confirmation dialog (only the removed person can undo it), and **hidden on backends that don't support it** (capability-detected via nodeInfo `software.name`; e.g. GoToSocial). See §20 for the rationale |
