...
**2. Persistent inline hint in Settings** (`DeviceNotificationSettings.html`, styled as a normal section description — a plain `<p>` inheriting `.ui-settings p`, *not* the small dimmed `device-notifications-note`; supersedes the old `intl.pushNotificationsNote`). The hint is shown only when this account is **not** the active push account (`!enabled`) and the two variants are **mutually exclusive**, so the active push account shows a clean state with no note: when a **different** account holds push, name it — *"Currently active for @A."*; otherwise, when more than one account is logged in, state the limit — *"OS push notifications are limited to one account per device."*
