...
Two lower-severity findings from the same review were also fixed: **(a)** the cache-first prefill made the initial list fetch route through the "new posts" buffer (possible spurious "Show X more" on first load) — `setupTimeline` now passes an `isInitialLoad` flag so the first fetch merges directly (see §12); **(b)** the settings master checkbox was set imperatively once in `oncreate` and didn't react to push being turned off elsewhere (circuit breaker / permission revoked) — it now observes the `enabled` computed, and the off-branch clears the subscription synchronously so the reactive checkbox doesn't flicker back on.

**Files:** `_store/store.js` (default `{}` + migration), `_actions/pushSubscription.js` (`canSilentlyReregister(instanceName)`, denied reconciliation, `markPushUnavailable`, `disablePushForInstance`, `enableOSNotificationsForInstance`), `_components/settings/instance/DeviceNotificationSettings.html` (`enabled` computed, reactive master checkbox, off-branch), `_actions/timeline.js` (`isInitialLoad`). Full mechanics in §18 / §12.
