...
| **1.9.0** | 2026-06-13 | **Consolidated release.** Bundles the new notification system (rich Web Push, honest single-account model C + C+, in-app notifications + sound, full i18n) with a full code-review hardening pass across nine neuralgic subsystems (push, streaming, compose, virtual-list, DB lifecycle, timeline read/hydration, auth/OAuth, service worker, word filters). Security: OAuth `state` CSRF protection on login. New: manage follows (follow/unfollow + "⋯" menu) directly from the Follows list, plus a scroll-to-top button on the follows/followers lists. Correctness fixes incl. timeline-read crash from dangling pointers, IndexedDB connection leak, empty-filter catch-all, and list/tag cold-load auto-retry. See the §22 Code Review Log for the per-area outcomes |
