...
*This handbook tracks the development line; the latest version it reflects is the most recent row in [§22 Version History](#21-version-history).*
...
- **Quality gate (v1.12.5).** The `Dockerfile` runs `pnpm run lint && pnpm test` before `pnpm run build`; a failure builds no image, so Watchtower keeps the running one (both workflows build from this Dockerfile). `pnpm lint` is `standard` (JS + HTML), `tsc` and `prettier`. It passed for the first time in v1.12.5. `pnpm test` runs the unit tests in `test/` (see CONTRIBUTING.md): `bin/run-tests.js` bundles each file with esbuild, swaps browser/IndexedDB/store modules for mocks declared in the file (`// @mock <import suffix> -> <mock>`), and runs it with `node:test`. They cover the areas that broke before; add a test with every fix there. Database tests run the real `_database` modules against `fake-indexeddb` (`import 'fake-indexeddb/auto'`), the only package in `devDependencies`; the build stage installs those because it runs with `NODE_ENV=development`. Every test has a 30 s timeout and the run exits when the tests are done (`--test-force-exit`), so a hanging test fails the build instead of stalling it (v1.12.6).
...
Zocial ships as a **backendless static bundle** served by nginx, so deployment settings can't reach the running app through ordinary `process.env` at runtime — anything read via `process.env` in the code is frozen by webpack's `DefinePlugin` **at build time**. To let an operator change settings on an existing image, `docker/entrypoint.sh` injects config **at container start** through two channels (both rebuild-free):

| Setting | Channel | Mechanism |
|---|---|---|
...
| `TRANSLATE_API` | **nginx** | `entrypoint.sh` `sed`s the URL into `nginx.conf` from `nginx.conf.template` (it must start with `http(s)://`, and characters that would break the substitution are refused with a clear error; `SINGLE_INSTANCE` likewise refuses characters that would break the JS string); nginx then reverse-proxies `/api/translate`, `/api/detect`, `/api/languages` to it (keeping any key server-side, no CORS). Swap the translation backend = set the env var + restart the container. See §13. |
...
**Several tabs/windows (v1.12.5).** Every tab reads localStorage once at load and `save()` writes the tab's *whole* value of each key it changed. Before v1.12.5 nothing was re-read, so a tab opened earlier reverted other tabs' changes when it saved: an account logged out elsewhere came back, one added elsewhere vanished, a posted draft reappeared, push changes were undone. `LocalStorageStore` now applies other tabs' `storage` events (and re-reads after a back/forward-cache restore) without marking the keys for saving, and fires `externalChange`. Rules:
- `currentInstance` stays per tab (`PER_TAB_KEYS`): each window may show another account.
- `composeData` merges per draft (`mergeExternal_composeData`): a draft this tab changed since it last synced the key (`ts` > `_syncedAt`) is kept, everything else follows the other tab, including a draft it posted and removed.
...
- Caches the open *promise* in `databaseCache` (keyed by instance) so repeated calls are fast and concurrent first-time callers share one open request instead of each opening (and leaking) a connection; a rejected open is evicted so a transient failure doesn't brick the instance.
...
`cache.js` keeps a small per-instance LRU (`QuickLRU`) in front of the statuses, notifications, accounts, relationships and meta stores. Writers (`cacheStatus`, the timeline inserts) fill it **synchronously, before** the background IDB write lands (§21 [v1.5.0]).

**Invariant: the status, notification, account and relationship caches never hold a miss.** `getStatus` / `getNotification` (v1.12.2) and `getGenericEntityWithId` (accounts, relationships; v1.12.3) cache only a found record, and only if no entry appeared while the IDB read was in flight (a concurrent insert has already cached the fresher network copy). Before v1.12.2 a miss was cached as `undefined`. After that, `hasInCache()` returned true while `getInCache()` returned `undefined`, and `doUpdateStatus` crashed on every `status.update` event for that post (`TypeError … reading 'id'`), and favourite/boost/bookmark on it failed after the server call had succeeded. The reply-header lookup in `Status.html` (the parent of a reply is usually not stored) made such misses common. `doUpdateStatus` also tests the cached value instead of `hasInCache()`. See §21 [v1.12.2]. Since v1.12.3 the reply header looks up the replied-to account instead (§21 [v1.12.3]), so the account cache follows the same rule: a cached miss or a clobbered entry would hide a header. Only the meta cache (`meta.js`) still caches misses; its callers only read.

### Failure handling (v1.12.4 review)

- **An aborted transaction rejects.** `dbPromise` also listens to `onabort`: a throw inside a request callback or a commit-time `QuotaExceededError` aborts without an `error` event, and the caller used to wait forever.
- **Another tab's logout isn't blocked.** Connections close on `versionchange`, and reading an open request's `result` while it is still pending (which throws) is avoided.
- **A logged-out instance's database stays deleted.** `deleteDatabase` (only logout calls it) marks the instance, and `getDatabase` refuses to re-open it for the rest of the page session. A fetch or stream event still in flight would otherwise re-create it with the logged-out account's posts. A new login always goes through the OAuth redirect, i.e. a fresh page load.
- **Records can be missing.** Statuses and accounts are cleaned up by their own timestamps, and search results are never stored. So the flag setters (`setStatusFavorited` etc.) skip a status that isn't stored, and `createMakeProps` drops a timeline item whose body or author is gone (from every in-memory timeline, incl. `notifications/mentions`) instead of rendering `undefined`, which would throw in a computed.
- **The UI doesn't wait on a successful write.** A failed IndexedDB write (quota) no longer swallows streamed posts, turns a follow/favourite the server accepted into an error, or undoes an optimistic update; the in-memory cache is filled before the write.
- **Cleanup runs at least every 30 minutes** (`maxWait`) — a busy home stream used to postpone the trailing debounce forever.
...
Since v1.12.4 an HTTP error's message also carries the server's own reason when the body is JSON: `Request failed: 422 · Validation failed: …` (best effort, 2 s cap on reading the body). Keep the `Request failed: NNN` prefix: `isNetworkNoiseError` matches it, and branching logic must use `err.status`, never the message. The text can come from the server, so escape it wherever it reaches `{@html}` (the login error does). Every endpoint module passes a timeout (boost, report and the push subscription calls didn't).

Mastodon's v2 media upload returns `url: null` (HTTP 202) while a large file is still processing. `waitForMediaProcessing` (`_api/media.js`) polls `GET /api/v1/media/:id` for up to 2 min; meanwhile `uploadingMedia` stays set, which also disables posting. Posting in between used to fail with 422, or publish the post without the file.
...
- **One socket at a time (v1.12.4).** The vendored client now closes the previous native socket before opening a new one, keeps at most one pending backoff timer, cancels it on `reconnect()`/`close()`, and ignores it after `close()`. Before, a reconnect during a pending backoff left two live sockets (every event delivered twice, two gap-fills), and a closed stream could re-open an orphaned socket that kept downloading, e.g. the federated firehose. A socket that is still connecting is left alone, and a non-JSON frame is ignored instead of throwing.
- **Keyed on the instance.** The active-timeline stream (`timelineObservers.js`) also restarts on `currentInstance`. Logout and account switches don't reset `currentTimeline`, so the old stream kept running with the old token.
...
`favorites` and `bookmarks` bypass this entirely and always use `fetchPagedItems`, which follows the `Link: rel="next"` header because Mastodon uses internal pagination IDs for those endpoints. A fresh fetch (first load, the 60 s poll, a revisit) reads the newest page and merges it on top; only "load more" continues from the cursor (before v1.12.4 every poll silently loaded the next *older* page), and a failed fetch shows a toast and an empty list instead of spinning forever.

**Bounded timelines (v1.12.5).** A tab left open for days kept every streamed post: tens of thousands of summaries per timeline, and each new post cost several full passes over the array (plus, until v1.12.5, a whole-array JSON comparison; `arraysEqual` compares by identity first). Now:
...
The design rationale for the choices below is summarised in §21; this section is the working reference for *how* it behaves.

### Backend

Default backend: `translate.zocial.social` (self-hosted LibreTranslate). Configurable in General Settings → Translation language. The proxy routes `/api/translate` and `/api/detect` to the configured backend, keeping the access key server-side. The proxy target is the `TRANSLATE_API` env var, substituted into the nginx config at container start (no rebuild) — see §4 *Runtime configuration*; operators swap the backend by setting `TRANSLATE_API` and restarting.
...
- **First visit follows the browser language.** On a fresh visit (no persisted `store_locale`), `store.js` matches `navigator.languages` against the shipped locales — exact tag first, then primary subtag (`de-DE`→`de`, `ru`→`ru-RU`, `en-GB`→`en-US`) — and starts in that language, falling back to `en-US` (`DEFAULT_LOCALE`) when none match. This happens **before** `setCurrentLocale` and the first paint, so the initial render is already correct (no flicker — the translatable landing UI is `HiddenFromSSR`/client-rendered) and the language dropdowns show it selected (`defaultValue={$locale}`). The detected language becomes the stored preference on the next save; an explicit dropdown choice always overrides it and is persisted (`store_locale`) + re-applied on boot.
...
> **Cross-locale placeholder trap (the subtlest bug we hit).** `getMessagesMap` decides "placeholder or not" **per the current locale's** value. A key can be **plain in en-US but a placeholder in another locale** — e.g. `followLabel` is `"Follow"` in en-US but `"Folgen {requested, select, …}"` in de/es/fr/ru. Read via `$messages`, it works in English and renders **`undefined`** in every other language (this is exactly what produced the "undefined" follow / hashtag buttons). **Rule:** a key is safe via `$messages` / a bare template `'intl.x'` only if it is non-placeholder in **every** shipped locale; otherwise it must go through `formatIntl`. The Python audit in §23 (parse `src/intl/*.js`, flag plain-in-en-US-but-placeholder-elsewhere) catches these — run it after adding `$messages` usages or new translations.

> **One key, two grammatical roles.** A second trap: a string that's reused for two purposes can need different wording per language even when en-US gets away with one word. `reblogged` served both the status-header label *and* the post-boost confirmation announcement; German needs present **"boostet"** for the header but participle **"geboostet"** for the announcement. The fix is a dedicated header-only key (`boostsAction`, `repliesTo`) rather than overloading one — see §23 (v1.10.0 row).
...
- **Pre-cache at install** — All webpack-emitted assets (`WEBPACK_ASSETS`) and static files (`ASSETS`) are cached with a version timestamp. Old caches are deleted on activate. Files without a content hash in their name (the app shell, `index.html`, theme CSS, icons) are fetched with `cache: 'no-cache'` (v1.12.5), so an old long-lived HTTP-cache entry can't be precached as the new version (see the nginx cache policy in §4).
...
- **Withheld quotes don't become cards (v1.12.4).** The client-side card resolver skips the fallback link whenever a Mastodon wrapper's state isn't `accepted` (`isQuoteWithheld`: pending, rejected, revoked, deleted, muted/blocked). A rejected or revoked quote is the quoted author's decision; rendering the post as a card anyway would undo it.
- **Delete-and-redraft keeps a Mastodon quote.** A native quote has no `quote_id`, and its source text doesn't contain the quoted URL, so the redraft appends it (the URL-in-text form of `quoteStatus`).
...
- **Once across tabs (v1.12.5):** every tab streams the same notification, so the sound takes a per-notification Web Lock (`ifAvailable`) and only the tab that gets it plays.
...
- **Friendly-fire teardown (fixed v1.8.5 — "N1").** `disablePushForInstance` (master toggle off) and `markPushUnavailable` (circuit breaker gives up) called `pushManager…unsubscribe()` **unconditionally**, destroying the *shared* mailbox. Disabling push on account A — or A's push service failing — silently killed OS push for B and C. It self-heals (B's next page load sees a null subscription and `canSilentlyReregister` re-registers), but only on the *next* load, and the re-register re-keys to B (breaking A again if A still wanted push). **Mitigation:** both teardown paths were guarded by `otherInstancesWantPush(instanceName)` — the shared subscription torn down **only** when no other logged-in instance still had `enableDesktopNotifications`. *(Superseded in v1.8.6 by the single-account model (C): only one account ever holds push, so the guard was removed and teardown is unconditional again.)*

- **v1.12.5: one push operation at a time, across tabs.** `updatePushSubscriptionForInstance`, `updateAlerts` and `disablePushForInstance` run under the Web Lock `zocial-push` (Web Locks aren't reentrant; code inside uses the `*Unlocked` variants) and first re-read the push state from storage. Two tabs loading together (session restore) could otherwise unsubscribe each other's fresh subscription, and a tab loaded earlier acted on stale state (re-enabling push someone turned off, or unsubscribing the subscription after push moved to another account). A server subscription whose endpoint differs from this browser's is re-registered.
- **v1.12.4 follow-ups.** Logging out the account that holds push now also unsubscribes the shared browser subscription (safe under the single-account model; a legacy state where another account still has a subscription record keeps it), so an offline logout can't leave the server pushing. `updateAlerts` checks the reused subscription's key against the server's VAPID key and re-keys on a mismatch (a leftover dummy or another server's subscription used to be registered as if valid). The silent self-heal respects the single-account model: if another logged-in account holds push, the stale intent flag is cleared instead of re-keying. `updatePushSubscriptionForInstance` reads the instance's own record, not the current instance's.
...
**Outcome (v1.8.6): C + C+ implemented** (the clean, fully client-side end state) — D was the interim. The single-account limit is now explicit and deterministic (the user picks the push account via a confirmation dialog), and the SW resolves `access_token`→instance so the active account gets rich notifications even with others logged in. **B** (relay) remains the only path to true *cross-instance* parallel push and is **out of scope** — its own server project, bundled with the self-hosted-services backlog (and, for Zocial, it would also need the account model to allow two accounts per instance). **The trap to avoid:** extending the per-instance illusion (more maps, more guards) as though it were real cross-instance parallelism — constraint #2 means it can never be in this app.
...
> Resolved in **v1.8.6** — C + C+ shipped (the options table above compares all four; see also §21 [v1.8.5]/[v1.8.6]). Kept here only as the reasoning that led there. "Multi-account" means accounts on **different instances** (the only multi-account Zocial allows).
...
**2. Two distinct hints in Settings** (`DeviceNotificationSettings.html`) — split by kind, because they are different things:
  - **Static constraint → a permanent `Tooltip`** (the focus-ring-style hover/tap component) embedded *in* the master toggle label: when more than one account is logged in the label renders as "Enable [OS Push Notifications](tooltip) on this Device" with the phrase **"OS Push Notifications"** as the dotted-underline trigger (single-account renders the plain label, no underline). The popup reads *"OS Push Notifications is only allowed for One Account per Device."* Independent of push state, so it never flashes or clutters the clean enabled state — this is the durable, discoverable explanation of the limit (`notifyOnThisDevicePre`/`notifyOnThisDeviceTrigger`/`notifyOnThisDevicePost` + `pushOneAccountPerDevice`).
  - **Dynamic status → an inline note** using the shared `settings-description` class (smaller + de-emphasized via `--deemphasized-text-color`, defined once in `SettingsLayout` and applied to *all* settings section descriptions): *"Currently active for @A."*, shown only on accounts that don't hold push (`!enabled` && another account holds it). The active push account shows no note (clean state).
  - (Supersedes the old `intl.pushNotificationsNote`.)
...
> **v1.12.5: per account.** The check below suppressed a push whenever *any* window was visible, even one showing another account. That account's stream doesn't carry the notification, so it was lost. Pages now report which account they show (`serviceWorkerViewObservers.js` → `postMessage({ type: 'zocial-view', instance })`, re-sent on account switch, visibility and worker change), and `pushIsInView` suppresses only when a visible window shows the push's account (resolved via the token lookup). Until a window has reported, which happens when the worker was just restarted, it counts as before.
...
Lists have grown into a subsystem of their own (CRUD, members, exclusivity, ordering, cache semantics) spread across the API/actions/DB/UI layers. This section is the map; the *why* behind each behaviour lives in the §21 entries linked inline.
...
- **Lists supported?** `syncLists` marks `instanceListsSupported=false` only on a definitive `403/404/501` from `GET /api/v1/lists`; transient errors (429/5xx/network) never hide the UI. Mirrors the §21 [v1.9.1] show-then-toast philosophy.
- **Exclusive supported?** Detected from the **presence of the `exclusive` key** on fetched List entities (`instanceListsExclusiveSupported`); unknown (zero lists) is treated optimistically. The create-dialog reads the field back off the created list and toasts if the server ignored it. See §21 [v1.10.5].
...
- **Members overview** (`/lists/:id/members`, `_pages/lists/[listId]/members.html`) — reuses `AccountsListPage` with `manageFollows` for follow/unfollow per member; §21 [v1.11.3].
...
**Size:** a stored entry is capped at 4000 characters (v1.12.5), and the places that logged whole statuses, stream frames or thread arrays log ids instead, so the log can't take the localStorage quota the app's state needs.

**Consequence for triage:** restored entries also survive an **app update**, but the version banner always shows the *current* build. After an update, a copied log can therefore mix entries from the previous release under the new banner. Before attributing an entry to the current version, check that its message format and chunk hashes match the current build. For example, the 2026-09-26 triage found `background refresh failed` printed with a full stack, which is the 1.11.0 format; since 1.11.5 only the message is logged.
...
**Files:** `_utils/isNetworkError.js`, `_utils/console/hook.ts`, `_actions/timeline.js`, `_store/observers/notificationObservers.js` — full mechanics in §20.
...
A related **partial-list notice** (`accountListPartial`, `partialNotice` computed) handles the *non-empty* truncated case: the list has rows, the server offered no `next` page (no `Link` header → `hasMore` false), yet `expectedCount > accounts.length` → "Showing X of Y — your server doesn't provide the rest." Followers/following paginate by an internal cursor that exists **only** in the server's `Link` header, so without it the client genuinely cannot page further — an honest notice rather than a futile "Load more". **This is also why the Load-more button shows on your own lists (server sends `next`) but not on foreign/restricted ones** — the code path is identical; the difference is purely whether the backend returns a next-page link.
...
### [v1.9.2] Purge an account's cached posts on unfollow (home + lists) / block (everywhere)

**Decision:** Remove that account's already-cached posts from the timeline cache — both the in-memory summaries/buffer (vanish immediately, no refresh) and the IndexedDB timeline pointers (so a cache-first cold load doesn't resurrect them). **Scope differs by action:** **unfollow → the follow-gated timelines: HOME + your LISTS** (you can only list accounts you follow, and unfollowing removes them from your lists server-side — so the same principle as home applies; *not* local/federated/tag/account, which show everyone regardless of following); **block → ALL cached status timelines** (home/local/federated/tag/list/account + open threads), because block means "nothing from them anywhere".
...
**Design choices:**
- **Unfollow = follow-gated timelines (home + lists).** Inclusion in home *and* in lists is gated by following (a list can only contain accounts you follow; unfollowing removes them from your lists), so both are purged. `removeAccountFromFollowGatedTimelines` filters the loaded in-memory `home` + `list/*` timelines, and the DB purge `deleteTimelineItemsForAccount(instance, accountId, { homeAndListsOnly: true })` scans the whole `status_timelines` store but only acts on keys starting `home\00` or `list/` (`isHomeOrListKey`). **This deliberately does NOT depend on `instanceLists`** (which is non-persisted, so it's empty in a fresh session) — the key filter covers *every* cached list, loaded or not, while leaving local/federated/tag/account untouched. **Block = everywhere:** `removeAccountFromAllTimelines` filters all cached in-memory timelines (`getAllTimelineData`) and the DB purge runs with no filter → whole `status_timelines` store.
...
- **Surgical — the cache is preserved (performance).** This is a targeted removal of *only* the affected account's pointers + in-memory summaries, **not** a cache wipe. All other accounts' entries (in-memory + IDB), the status bodies, and the status/account/`itemHeights` caches stay intact — so the rest of the timeline keeps rendering cache-first with **no re-fetch and no re-measurement**, and the "instant render" benefit is retained for everyone else. Keeping bodies is also deliberately conservative: if a purged post is referenced elsewhere (thread/notification) or returned again by the server after refollow/re-add, the body is often still cached → no re-download.
...
- **Unblock does NOT auto-restore.** Block *deleted* the cached posts (in-memory + IDB pointers), so nothing is restored from local cache — content only comes back from the server, and the two paths differ:
  - **A refresh/refetch** (navigation / 60 s poll / reload) re-fetches the *current* server window of that timeline, now unfiltered. So the account's posts that are still in the recent window reappear — **including ones made while blocked** (they were always on e.g. the federated timeline, just hidden from you), not only post-unblock posts. It is **not** a restore of your deleted cache: a specific older post that has since scrolled out of the recent window won't return via a top refresh (only via paginating further, where the server now returns it unfiltered).
  - **Streaming alone** (no refresh) only delivers genuinely **new** posts from then on.
  We deliberately do **not** auto-refresh on unblock: the gain is small (unblock is rare) and a forced refresh would surface via the standard path anyway (direct at the top, "Show X more" when scrolled — never a mid-scroll jump). Reload/navigation brings the current window back. **Rationale (don't relitigate):** block is durable intent, rarely reverted immediately, so instant cache-restore has little value — and enabling it would mean *not* purging on block (keeping the content cached), which contradicts what block should do. Purge-on-block + fetch-on-demand is the coherent model and matches the app's fetch/stream-driven timeline (no reversible local diff to maintain).
- **Refollow is treated like a fresh follow, not a restore.** We purge on unfollow and never re-add on follow, and the deleted home/list pointers mean cache-first can't resurrect the old entries. After a refollow the account's *recent* posts reappear only via the server's home backfill on the next fetch — exactly as when following someone new. This is the correct/expected behaviour (no stale pre-unfollow history snaps back). **Lists:** unfollow removes the account from your lists *server-side*, and Mastodon does **not** restore list membership on refollow — so the account must be manually re-added to a list (⋯ → "Manage list memberships"); Zocial intentionally doesn't try to restore membership. **Verified (don't re-investigate):** after re-adding an account to a list, its *recent* posts reappear in the list — this comes from the **server**, not our cache. GoToSocial (e.g. social.ztfr.eu, GtS 0.21.2) doesn't materialise feeds; it **live-queries** the list timeline from members' statuses, so a re-added member's recent posts (incl. ones from before the re-add, within the current window) are returned by `GET /timelines/list/:id` (confirmed in the response JSON during testing, June 2026). Our purge correctly cleared the cache; these are legitimate server-returned list contents and must **not** be filtered client-side.

**Files:** `_database/timelines/deletion.js` (`deleteTimelineItemsForAccount` + `isHomeOrListKey`; whole-store cursor, `{ homeAndListsOnly }` key filter), `_actions/timeline.js` (`purgeAccountFromTimelineInMemory`, `removeAccountFromFollowGatedTimelines`, `removeAccountFromAllTimelines`), `_actions/follow.js` (home+lists purge on unfollow; network-noise → warn), `_actions/block.js` (all-timelines purge on block; network-noise → warn), `_components/AccountsListPage.html` (`refreshAccountsList` handler now `.catch`es).
...
> The original "always start in English" default was later refined to follow the browser language on first visit — see the **[v1.10.2] First-visit language detection** entry below.
...
- **Net delta** for the only theoretically-remaining case (installed PWA + a server with *no* Web Push + desktop + window open-but-unfocused) is tiny and practically near-zero (desktop PWAs almost always have working push); the known Firefox-mobile push wobble doesn't qualify either (mobile background ⇒ frozen stream). Against that: a second permanent delivery mechanism (lifecycle on the toggle, dedup, teardown, a notification-content builder shared with the SW) plus §18/§21 churn. Cost/benefit is clearly negative.

**If revisited:** the trigger would be a **persistent, non-Electron host without Web Push** becoming a real target. The design is known and recorded here: single active path with clean teardown, dedup by notification id, a per-type title/body + HTML-strip + click-to-focus builder **shared** with the service-worker push handler, path chosen once at enable-time (runtime push→fallback transition as a later TODO hooked on the existing circuit breaker). **Two things a *polling* fallback would need that the current push/streaming model does not** (raised when the v1.11.2 push avatar-icon landed): (1) a **poll reentrancy guard** — a `lastNotificationId` cursor plus an "already running" flag, so overlapping poll ticks (made longer by any per-notification avatar/enrich fetches) can't double-fire notifications or race the cursor; (2) **avatar caching** is *not* needed even then — notification icons are cross-origin media URLs that the browser HTTP-caches per the media host's headers; caching other origins' avatars in our own cache store would be storage bloat + a privacy smell. The push/streaming paths today need neither: each push is an independent SW event deduped by `tag`, and the foreground path is a single streamed event (sound only).

---

### [v1.10.1] List/tag cold-load auto-retry now also covers 5xx (deliberate 500 refetch)

**Decision:** The single automatic retry for list/tag timeline fetches (`fetchTimelineItemsFromNetworkWithRetry`, see the §23 2026-06-13 row) now fires not only on **status-less transient errors** (timeout / failed fetch) but **also on 5xx server errors** — `isTransient = isNetworkNoiseError(e) && (!e.status || e.status >= 500)`. 4xx stay non-retried (deterministic).
...
1. **Synchronous re-render for already-seen items** — cache `makeProps` output per item id and skip the idle-task gate on re-mount, so a re-entering item renders fully at once (no "empty → filled" gap). Touches the deliberately-tuned lazy-render path (§23 virtual-list reviews).
...
**Decision (this is distinct from the already-hardened *main* fetch path — `fetchTimelineItems`, see §21 [v1.10.1] + §23):**
...
### [v1.10.5] Exclusive lists + a "Manage lists" page (rename / delete / exclusive toggle)

**Feature:** Support Mastodon's/GoToSocial's **exclusive lists** — a list whose members' posts are hidden from the home timeline (read them only in the list). Added as (a) a checkbox in the create-list dialog and (b) a new **Manage lists** settings page where each list can be renamed, deleted (with confirmation), and toggled exclusive.

**Capability detection — clean, no software allowlist.** `exclusive` is a field on the List entity (Mastodon 3.3+, GoToSocial — verified in the GtS swagger: "If true, hide posts from members of this list from your home timeline"). A backend that supports it returns the `exclusive` key on **every** List entity; one that doesn't omits it. So support = "any fetched list carries the `exclusive` key" (`instanceListsExclusiveSupported`, set in `syncLists` / from the create+update responses). With **zero** lists we can't tell, so the flag stays `null` (unknown) and is treated optimistically (`!== false`), exactly like the existing `listsSupported` gate. This is the deliberately-preferred alternative to a version allowlist (cf. §21 [v1.9.1] `remove_from_followers`). The create-dialog checkbox additionally **reads the field back** off the created list: if the user asked for exclusive but the server didn't honour it, a toast says so (the list is still created) and the now-known `false` flag hides the checkbox thereafter.

**Cascading UX (deliberate).** Visibility staggers cleanly with no extra persistent state: lists unsupported → the whole lists area is hidden (existing behaviour); lists supported → "Create list" shows; **≥1 list exists** → the "Manage lists" entry appears (gated `listsSupported !== false && lists.length > 0`). Deleting the last list symmetrically removes the entry again. To avoid a dead-end, the Manage-lists **page itself stays fully usable at zero lists** (empty state + an inline "create list" field like the followed-hashtags page), so a user who deletes their last list there can immediately re-create one.

**Home timeline sync (extended in v1.10.7 — see below).** Initially only list *deletion* marked home stale; the full home-sync (purge an exclusive list's members from home immediately, mirroring unfollow/block) was added in **[v1.10.7]**.

**Key files:** `_api/lists.js` (`createList(+exclusive)`, `updateList` PUT, `deleteList` DELETE), `_actions/lists.js` (support detection, `createNewList`/`renameList`/`setListExclusive`/`deleteListById`), `_store/store.js` + `instanceComputations.js` (`instanceListsExclusiveSupported` / `listsExclusiveSupported`), `_components/dialog/components/CreateListDialog.html` (checkbox + read-back toast), `_components/settings/instance/ManageListsSettings.html` (new), `routes/manage-lists.html` + `_pages/manage-lists.html` (new), `_components/settings/CommunitySettingsSection.html` (menu entry), `intl/*` (new keys, all 5 locales).

---

### [v1.10.7] Exclusive lists purge from home (like unfollow); alphabetical list order; Manage-lists polish

**Exclusive → home purge (the substantive part).** The exclusive promise — "members of this list won't appear in your home timeline" — only held for *new* posts: already-cached member posts lingered, because the home cache is **union-only and never unmerges** (the exact gap fixed for unfollow/block in [v1.9.2]). Marking home stale is **not** enough — a fresh home fetch is server-filtered but the union merge keeps the stale entries — so we now do an explicit **home-only** purge, mirroring unfollow but never touching the list's own timeline (members must stay in the list):
- **Turn a list exclusive / add an account to an exclusive list** → purge the affected member(s) from home (in-memory `home` summaries + IDB `home\u0000…` pointers via `removeAccountsFromTimeline(…, 'home', ids)`). Toggling on fetches the members first (`getListAccounts`, `limit=0` = all, no pagination).
- **Turn it non-exclusive / remove a member / delete the list** → mark home **stale** so the next fetch brings them back, server-filtered (no retroactive cache rebuild — §21 [v1.9.2] model).

Membership changes are routed through `addAccountToListAndPurge` / `removeAccountFromListAndRestore` (used by `ListMembershipDialog`) so exclusivity is respected wherever membership is edited. **Not like mute/block in one way:** membership is dynamic, so the sync hooks several points (toggle, add, remove, delete) and is **home-only**, not account-wide. This reverses the v1.10.5 "home behaviour is fine as-is" call — deliberately, because it now matches what the UI tells the user.

**Alphabetical list order (instead of drag-and-drop reorder).** There is **no server-side list order** (verified: neither Mastodon nor GoToSocial expose an order/reorder field), so a manual reorder could only be a per-device, non-synced client preference. Rather than build touch+mouse drag-and-drop for a device-local order, lists are sorted **alphabetically by title** consistently — already so on the Manage-lists page, now also in the community settings menu (`sortedLists`). Predictable and order-stable across views without extra state.

**Manage-lists polish.** "Manage lists" moved **above** "Followed hashtags" in instance settings; the create-list field + button (and, for parity, the followed-hashtags field + button) enlarged to match the profile "Edit profile" button (`height: 36px; font-size: 1em`).

**Key files:** `_actions/timeline.js` (`removeAccountsFromTimeline`), `_database/timelines/deletion.js` (`deleteTimelineItemsForAccounts` with the `{ timelineName }` / `{ homeAndListsOnly }` scopes), `_api/lists.js` (`getListAccounts`), `_actions/lists.js` (`setListExclusive` purge/mark-stale, `addAccountToListAndPurge`, `removeAccountFromListAndRestore`), `_components/dialog/components/ListMembershipDialog.html`, `_components/settings/CommunitySettingsSection.html` (`sortedLists`, menu order), `_components/settings/instance/{ManageListsSettings,FollowedHashtagsSettings}.html` (sizing).
...
**Discoverability.** Each nav link is associated (via `aria-describedby`) with a shared visually-hidden hint (`#nav-reorder-hint` in `Nav.html`, text `tabReorderHint` = *"Reorder with Alt + Shift + arrow keys"*), and the shortcut is also listed in the in-app **hotkey help** (`globalHotkeys`, all 5 locales). `preventDefault` suppresses the browser's Alt+Arrow back/forward; focus is kept on the moved tab (the keyed `{#each}` reuses the node; a defensive `preventScroll` refocus follows the re-render).

**a11y refinement (v1.11.6) — describedby over keyshortcuts.** Originally each link carried `aria-keyshortcuts="Alt+Shift+ArrowLeft Alt+Shift+ArrowRight"`. The blind tester reported that **NVDA in browse mode announces `aria-keyshortcuts` *before* the accessible name** (she heard "Alt+Shift+Right Alt+Shift+Left, Benachrichtigungen" — shortcut first, tab name last). The announcement order of `aria-keyshortcuts` is AT-controlled and can't be overridden; `aria-describedby`, by contrast, is announced **after** the name + role. Switched to a single shared `aria-describedby` hint so the **tab name comes first**, then the shortcut. The actual key binding (`Nav.html` `onKeydown`) is unchanged — only the announcement metadata moved.

**Considered, not built:** a separate "move up/down buttons" reorder panel in settings (more discoverable for screen-reader users who don't know the shortcut, and the WAI-ARIA "buttons" variant). Deferred as overkill — it would need its own settings surface; the in-nav hotkey + the spoken hint + help entry covers the need in place. Could be added later if the shortcut proves hard to discover.

**Key files:** `_components/Nav.html` (`onKeydown` → `_reorderTabs` + `announceAriaLivePolite`; the shared `#nav-reorder-hint`), `_components/NavItem.html` (`aria-describedby`), `_utils/announceAriaLivePolite.js` (reused), `intl/*` (`tabReordered` announcement, `tabReorderHint`, + a `globalHotkeys` line).
...
### [v1.12.2] Status/notification caches hold found records only (no negative caching)

**Symptom (production 1.12.0 log):** dozens of `⛔ Uncaught (in promise): TypeError: Cannot read properties of undefined (reading 'id')` at `cacheStatus` ← `doUpdateStatus`, one per streaming `status.update`. Not visible in the log: favourite, boost or bookmark on the affected post failed with a toast after the server had already accepted it (`reading 'favourited'`), and the edit never reached IDB.

**Cause:** `getStatus` / `getNotification` cached a miss as `undefined` (inherited from Pinafore). `QuickLRU.has()` then returns true while `get()` returns `undefined`, and `doUpdateStatus` trusted `hasInCache()`. Upstream a miss was rare. In Zocial it is common since the reply header (2026-05-31) asks `Status.html` for the parent of every reply, and the parent is usually not stored. A second path: since the background IDB writes (§21 [v1.5.0]), an insert caches a post synchronously and writes IDB later, so a read that started before the insert finds nothing in IDB and then overwrote the fresh cache entry with `undefined`.

**Decision:**
1. Cache a read result only if a record was found **and** no entry appeared while the read was in flight. Anything written during the read comes from a concurrent insert and is at least as fresh as what IDB returned.
2. Keep the invariant "the status/notification caches never hold `undefined`" and document it (§7). `doUpdateStatus` additionally tests the cached value rather than `hasInCache()`, as a second line of defence.

**Tradeoffs** (both mostly superseded by §21 [v1.12.3]: the reply header no longer looks up the parent post, and the account/relationship caches follow the same rule): (a) A repeated lookup of a post that isn't stored now costs one read-only IDB transaction each time (the reply-header lookup on every mount of such a reply). That cost is small and async, and the alternative (a negative cache) goes stale as soon as the post arrives by a path that doesn't call `cacheStatus`, such as the IDB branch of `doUpdateStatus`. (b) The account/relationship caches (`getGenericEntityWithId`) still cache misses. Their callers only read and fall back to the network, so a stale miss costs a refetch, not a crash; left as is to keep the change narrow.

**Files:** `_database/timelines/getStatusOrNotification.js`, `_database/timelines/updateStatus.js`. See §7, §23 (2026-09-26).

---

### [v1.12.3] Reply header: look up the replied-to account, not the parent post

**Problem:** The "↩ reply to [avatar] Name" header (added 2026-05-31) loaded the **parent post** from IDB only to read its `account`. The parent of a reply is often not stored, so whether the header appeared depended on invisible cache state. The same reply could render without the header and later with it, for example after the thread had been opened once. Every lookup was also a two-store transaction plus `cloneDeep`, and it was the main source of the cached misses behind §21 [v1.12.2].

**Decision:** The header needs only the replied-to account, so resolve that directly:
1. **Self-reply** (`in_reply_to_account_id` = the author) → the status's own `account`. **Reply to me** → `currentVerifyCredentials`. No lookup and no async re-render; the header is present before the first height measurement.
2. **Otherwise** → `database.getAccount(in_reply_to_account_id)`: one store, cached, no clone. Accounts are stored whenever any of that person's posts, notifications or profile passed through the client, which is far more often than one specific parent post.
3. **Fallback:** the old parent-post lookup, only for servers that send `in_reply_to_id` without `in_reply_to_account_id`.

`getGenericEntityWithId` now also caches found records only and never overwrites an entry written during the read (the §21 [v1.12.2] rule), because the header now depends on the account cache.

**Tradeoffs:** (a) Still not 100 %: a reply to an account the client has never stored shows no header, as before. A fallback to `@acct` from `mentions` was rejected: it would render a second, avatar-less header variant, and the mention is missing when the author removed it. (b) The stored account can be slightly stale (old avatar or display name), as everywhere else accounts are shown from the local store. (c) A reply to a muted or blocked account could show that person in the header. Mastodon already removes such replies from home server-side (replies to non-followed accounts and mentions of muted/blocked accounts are filtered), and the old parent lookup had the same exposure whenever the parent was stored. No client-side guard: relationships are only known locally for visited profiles, so it would be unreliable.

**Files:** `_components/status/Status.html` (oncreate reply lookup), `_database/helpers.js` (`getGenericEntityWithId`). See §7.
...
| **1.8.7** | 2026-06-13 | Virtual-list review pass (no behaviour change): removed leftover debug `console.log`s from the scroll/fullscreen/realm paths, and the throttled scroll listener is now cancelled on teardown so a trailing invocation can't write into a null realm after the list is destroyed |
| **1.8.8** | 2026-06-13 | IndexedDB lifecycle review: `getDatabase` deduplicates concurrent first-time opens by caching the in-flight promise — fixes leaked DB connections on cold load that could later block `deleteDatabase` on logout (failed opens are evicted, not cached). Removed unconditional debug `console.log`s from the DB write/cleanup/freeze paths |
| **1.8.9** | 2026-06-13 | List/tag load reliability: a single automatic retry (a `fetch()` only — never a page reload) now fires when a list or tag fetch hits a transient error (timeout / failed fetch) while the view is blank or stale — automating the manual refresh users were doing, so a slow first server-side list assembly recovers in seconds instead of after the 60 s poll. Fresh visible content is never re-fetched over; a successful retry is buffered behind "Show X more" if scrolled. Graceful fallback on a second failure is unchanged |
| **1.8.10** | 2026-06-13 | Timeline read hardening: `getTimeline` drops `undefined` holes from dangling timeline pointers (a timeline row whose status/notification body failed to store), which previously crashed `timelineItemToSummary` and blanked the timeline. Removed per-stream-item debug logging |
| **1.8.11** | 2026-06-13 | Auth/OAuth review: added an OAuth `state` CSRF token to the login flow (generated before the authorize redirect, persisted across it, and verified on the callback before the code is exchanged). Fixed logout clearing the active instance's virtual-list cache instead of the logged-out instance's |
...
| **1.10.3** | 2026-06-17 | **No layout shift when a dialog opens** (dev patch). `scrollbar-gutter: stable` on the root scroll container reserves the scrollbar's width permanently, so the `<html>` scroll lock (from 1.10.1) no longer reclaims ~15 px and slides the centered layout when a dialog/context menu opens or closes. Affects classic space-occupying scrollbars (Electron desktop, Linux/Windows Chromium, Linux Firefox); no effect on overlay-scrollbar platforms (macOS/mobile), so nothing is forced visible — see §21 |
| **1.10.4** | 2026-06-19 | **List timeline reliability — streaming gap-fill hardened** (dev patch). The stream's catch-up fetch (`fillStreamingGap`, fired on open/reconnect) was an unguarded, fire-and-forget loop of up to 15 sequential requests — brutal on the slow, per-list `GET /timelines/list/:id` query and the source of the `Uncaught (in promise): NetworkError` users saw. It now: wraps the fetch in try/catch (graceful degrade + warn-vs-error), caps slow list/tag catch-ups at 2 requests (vs 15), and uses the smaller list batch. Explains why lists fail where federated doesn't (heavy per-list query vs cheap public read) — see §21 |
| **1.10.5** | 2026-06-19 | **Exclusive lists + Manage-lists page** (dev patch / feature). New lists can be marked **exclusive** (their members' posts are hidden from your home timeline and shown only in the list) via a checkbox in the create dialog, and a new **Manage lists** page (under Instance settings, appears once you have ≥1 list) lets you rename, delete (with confirmation), and toggle exclusive per list. Capability is detected cleanly via the `exclusive` field on the List entity (Mastodon 3.3+, GoToSocial) — no software allowlist; unsupported → feature hidden / toast on the first create. Deleting an exclusive list marks home stale so its members reappear — see §21 |
| **1.10.6** | 2026-06-20 | **Lists/timelines render immediately on open** (dev patch / fix). A freshly-opened timeline — most visibly a brand-new/empty list — could show nothing (not even the loading footer or empty state) until a manual scroll/resize. The virtual list measured its viewport/scroll metrics only synchronously at mount (before the route's layout settled) and never re-measured; stale metrics collapsed the visible-items window. Now it measures unconditionally at mount, re-measures once after layout settles, and again when items first arrive — see §23 |
| **1.10.7** | 2026-06-20 | **Exclusive lists hide members from home immediately + list management polish** (dev patch). Making a list exclusive (or adding an account to one) now **purges those accounts' already-cached posts from the home timeline** right away — mirroring unfollow/block — instead of leaving them until they aged out; making it non-exclusive / removing a member / deleting the list brings them back. Lists are sorted **alphabetically** in the settings menu too (matching the Manage-lists page; there is no server-side list order, so no drag-reorder). "Manage lists" now sits above "Followed hashtags", and the create-list / add-hashtag fields + buttons are enlarged to match the "Edit profile" button — see §21 |
| **1.10.8** | 2026-06-20 | **Removing an account from a list clears its posts from that list** (dev patch / fix). Deselecting a list in the profile "⋯" → manage-list-membership dialog removed the membership server-side but left the account's already-cached posts in the list feed — a long-standing regression since list timelines became cache-first (1.8.4). Removal now purges that account from the list's timeline (in-memory + IDB), mirroring the unfollow/block purge scoped to one list — see §23 |
...
| **1.12.6** | 2026-09-26 | **Test coverage (dev patch / tests + fixes).** 75 more unit tests (122 in total): all locales against en-US (keys, message syntax, placeholders, every `'intl.x'` used in the code), word filters, ID sorting and thread order, push registration, the service worker's push handler, the OAuth login, the hashtag bar, MFM, translation language detection, and the database layer against a real IndexedDB implementation (`fake-indexeddb`, the first `devDependency`): schema and v9 migration, paging, threads, deletion, cleanup, logout and other tabs. **Found and fixed:** the Spanish, French and Russian tab title said "Zocial" where the others show the site's host name; the direct replies of an opened post were not sorted after cached and fresh replies were merged. The test runner now fails a test that never settles instead of stalling the image build. See §4, §23 |
...
> **Note:** these are point-in-time records. Function/file names reflect the code **as it was on that date** and may have since been renamed or consolidated (e.g. the single-timeline purge helpers were later folded into `removeAccountsFromTimeline` in v1.11.2). Treat the dated rows as history, not as a current API reference — for current names see the relevant section above and §21.
...
| 2026-06-12 | **Timeline cache-first** | initial-load direct-merge vs. buffer, stale / 30 s-throttle interaction, list/tag slow-endpoint mitigations | Clean; `isInitialLoad` direct-merge confirmed (no spurious "Show X more"); T1 null-guard added. ✅ |
| 2026-06-12 | **Svelte 2 footgun sweep** (changed `.html`) | computed/`oncreate` throws that blank the page, `formatIntl` / intl rules | Green — all guarded. ✅ |
| 2026-06-12 | **State store / migrations / logout** | per-instance maps, boolean→map migration, `logOutOfInstance` cleanup | Clean; `enableDesktopNotifications` correctly survives logout; prompt-flag asymmetry harmless (permission guard). ✅ |
| 2026-06-13 | **Streaming + timeline buffer** | `TimelineStream` ws lifecycle (freeze/thaw, reconnect, online/offline), `freshUpdates`/`timelineItemSummariesToAdd` buffer merge/dedup, `fillStreamingGap`, scroll-aware drain | Solid; cache-first/notification integration confirmed clean. Fixed: #1 double-connect churn + #2 needless teardown of an open socket (unified into `_ensureConnected`), #3 unguarded inner `JSON.parse` in `processMessage`. Accepted: #4 gap-fill can buffer up to ~300 items → "Show X more" (intended). ✅ |
| 2026-06-13 | **Compose system** | post/submit path (`compose.js` `postStatus`), data model (`composeMixins`), media upload (`media.js`), polls/redraft (`composePoll`, `deleteAndRedraft`), `ComposeInput` (textarea), Svelte-2 sweep | Solid: textarea-based (no contenteditable risk), Svelte-2 sweep green, strong double-post guard, `postStatus` preserves the draft on failure. **All 5 findings fixed:** **C1** `deleteAndRedraft` now loads the dialog + resolves the reply handle (non-fatal) *before* the destructive delete (no data-loss window); **C2** poll-only posts now allowed, and invalid/empty posts toast instead of a silent dead click (`cannotPostEmpty` / `pollNeedsTwoOptions`, translated); **C3** `deleteMedia` clears the cached media blob; **C4** a failed media-metadata PUT no longer aborts the post (also stopped mutating stored focal-point data); **C5** `throttleTimer` is now per-`ComposeInput` instance (no cross-box save overwrite). ✅ |
| 2026-06-13 | **Virtual-list internals** (`_components/virtualList/*`, `RealmStore`, `reselect`) | height measurement (batch + on-demand recalc), `rawVisibleItems` windowing maths, `reselect` re-render guard, realm LRU cache, scroll restoration, freeze/fullscreen handling, Svelte-2 store-observer leak hack | Solid, mature Pinafore code — no correctness bugs. The store-observer leak is already worked around (component-mirror computeds), in-place `itemHeights` mutation is fine (Svelte-2 `set` recomputes by provided key, not identity), and the `height:0` collapsed-bundle case is deliberately handled (`=== undefined`). **Fixed:** removed leftover debug `console.log`s (incl. one firing on every scroll-restore height-completion and one on every fullscreen toggle); `teardownScroll()` now `.cancel()`s the throttled scroll listener so a trailing call can't write `scrollTop`/`scrollHeight` into a null realm after `ondestroy`. **Noted (both revisited 2026-06-14 — see row below):** per-realm `itemHeights` growth, and a `requestPostAnimationFrame` height write firing for an already-destroyed item. ✅ |
| 2026-06-13 | **Max-effort sweep of the whole `dev`↔`main` diff** (all 9 prior passes + the unreviewed push/notification subsystem: `pushSubscription.js`, `DeviceNotificationSettings.html`, `store.js` migration, `pushToken*`, `showDesktopNotification.js`) | recall-mode line-by-line + cross-file scan of the full branch diff (46 files); ran as a local fallback for the cloud `ultrareview` | No new correctness bugs — the `enableDesktopNotifications` boolean→map migration, OAuth `state` flow, token-routing reconcile chain, and re-key dialog all verified sound. **Fixed:** the device-notifications master checkbox was set imperatively only in `oncreate`, so it could go stale if push was silently given up while settings were open — now a component `observe('enabled', …)` keeps it in sync (auto-cleaned). Removed the remaining frequent debug `console.log`s in `timeline.js` (11), `TimelineStream.js` (7), and `ComposeBox.html` (1). **Confirmed (no secrets logged.) Carry-overs unchanged:** `getNotification` by-reference caching, `deleteDatabase` `onblocked` never settling. ✅ |
| 2026-06-13 | **Word/content filters** (`_utils/createRegexFromFilters.js`, `escapeRegExp.js`, `createFilterFunction.js`, `computeFilterContextsForStatusOrNotification.js`, `_store/computations/wordFilterComputations.js` + `timelineFilterComputations.js`, `_store/observers/wordFilterObservers.js`, `_actions/filters.js`) | regex building (drop vs. warn buckets, whole-word, context split), per-summary `filterContexts` precompute + recompute-on-change, expiry handling, `.test()` usage | Well structured: drop/warn buckets are split correctly, `filterContexts` are precomputed on summaries for perf and recomputed when filters change, `.test()` is stateless (no `g` flag → no `lastIndex` bug), and expiry is `isEqual`-guarded so the 10 s `now` tick doesn't thrash. **Fixed (F-1, latent catastrophic):** an empty filter phrase produced an empty regex alternative (`foo|`) that matches *everything* — silently hiding the entire timeline. `createRegexFromFilters` now drops empty expressions and returns a never-matching `/(?!)/` if none remain. Removed per-filter-change debug `console.log`s. **Noted (not changed):** a whitespace-only phrase still matches broadly, but that's explicit user input, not a footgun. ✅ |
| 2026-06-13 | **Service worker** (`service-worker.js`) beyond push | install/activate cache lifecycle (asset versioning, keep-2-webpack-builds, `skipWaiting`/`clients.claim`), `fetch` routing (cache-first shell, on-demand font/wasm caching, manifest/theme/share-target handling), push enrich + `notificationclick` actions | Solid: cache versioning and the keep-two-webpack-builds cleanup are correct, every handler stays inside `waitUntil` with a guaranteed fallback (bare notification on bad/empty payload so a `userVisibleOnly` push is never silently dropped → no subscription revocation), and `'intl.*'` action labels **are** build-time translated (the `svelte-intl-loader` runs over `service-worker.js`, not just `.html`). **Fixed (S-5, correctness):** the `mention` notification derived the instance API origin from the **avatar URL** (`new URL(data.icon).origin`) — wrong whenever avatars live on a separate media/CDN domain, sending the reblog/favourite `notificationclick` POST to the wrong host. Now passes the instance origin already resolved from the push token (C+) through to `showRichNotification` (also removes a `new URL(undefined)` throw risk when a payload has no icon). **Noted (not changed):** `notificationclick` default always `openWindow`s rather than focusing an already-open client (minor UX); enrich GET timeout is a deliberate 2 s (falls back to a simple notification). ✅ |
| 2026-06-13 | **Auth / OAuth & instance management** (`_actions/addInstance.js`, `instances.js`, `_api/oauth.js`, `_api/utils.js`, `_utils/ajax.js`, callback `_pages/settings/instances/add.html`) | OAuth authorization-code flow (dynamic per-instance app registration → authorize redirect → code exchange), token storage, instance switching, full logout teardown, 401→auto-logout | Flow is sound: dynamic per-instance client registration binds the code exchange to the user's own `client_secret` (so classic login-CSRF is already hard), `getAccessTokenFromAuthCode` uses `application/x-www-form-urlencoded` correctly, logout teardown is thorough, and `logOutOnUnauthorized` cleanly handles revoked tokens. **Fixed (A-1, security):** the authorize request carried **no OAuth `state`** (RFC 6749 §10.12 CSRF token) — added a random `crypto`-generated state, persisted across the redirect and verified against the callback's `state` before the code is exchanged; a missing/forged state is refused. **Fixed (A-6):** logout cleared the *active* instance's virtual-list realms (`currentInstance + '/'`) instead of the **logged-out** instance's (`instanceName + '/'`) — wrong cache cleared when logging out a background account. **Noted (accepted, architectural):** access tokens live in `localStorage` (inherent to a backendless SPA; XSS-exposed) and the single-use `?code=` stays in the URL/history on a failed callback — low risk, not changed. **Verified:** login + logout manually tested by the user and working (the state-mismatch / denied-login edge paths not specifically exercised). ✅ |
| 2026-06-13 | **Timeline read / hydration** (`_database/timelines/pagination.js`, `lookup.js`, `fetchStatus.js`, `fetchNotification.js`, `getStatusOrNotification.js`, `_actions/addStatusOrNotification.js`, `rehydrateStatusOrNotification.js`) | async-cursor read pattern (`callback(res)` before inner gets fill it), key-range pagination, fresh-update buffer merge/dedup, rehydration markers + DB-store guard | Read pattern is correct (inner `get()`s keep the tx alive until `oncomplete`); the `[rehydrated]` symbol guard in `insertion.ts` correctly stops hydrated objects reaching the DB; buffer dedup via `uniqById` is sound. **Fixed (T-1, latent crash):** `getTimeline` now drops `undefined` holes from dangling timeline pointers — a timeline row can outlive its status/notification body (the body's `put()` can fail structured-clone in `insertion.ts` and be skipped while the pointer is still written), and `fetchStatus`/`fetchNotification` then call back `undefined`; returning those crashed `timelineItemToSummary` (`item.id`) and blanked the timeline. Removed per-stream-item debug `console.log`s. **Noted (not fixed):** `getNotification` returns/caches by reference (no `cloneDeep`), unlike `getStatus` — a shared cached object that callers/rehydration mutate in place; low impact (in-memory cache, store-guard prevents DB leakage) but an asymmetry worth revisiting. ✅ |
| 2026-06-13 | **List/tag cold-load timeouts** (`_actions/timeline.js`, `_api/timelines.js`, `timelineObservers.js`) — user-reported: lists often time out on the first fetch, a manual refresh fixes it | initial cold fetch path, 40 s `SLOW_READ_TIMEOUT`, empty-cache fallback, 60 s poll self-heal | Root cause is mostly backend: list/tag timelines are assembled server-side and the *first* cold request is slow (GoToSocial warm-up), timing out at 40 s. The app already self-heals via the 60 s poll, but only after a long blank spinner. **Fixed:** added a single automatic retry (just another `fetch()`, never a page reload) for a list/tag fetch on a transient error (timeout / failed fetch, no HTTP status) when the view is **blank or stale** — automates the manual refresh, recovering in seconds instead of waiting for the next poll tick. The retry gate reads stale-ness *before* the fallback marks it, so fresh visible content is never re-fetched over; a successful retry result is held behind "Show X more" if the user scrolled, or merged at the top on initial load (no scroll jump). If the retry also fails, the existing graceful fallback (empty/cached list, no toast) is unchanged. Worst case (backend down) is one extra request, bounded. ✅ |
| 2026-06-13 | **IndexedDB / DB lifecycle** (`databaseLifecycle.ts`, `migrations.js`, `cache.js`, `cleanup.js`, `insertion.ts`, `knownInstances.js`) | per-instance open/close/delete, upgrade migration chain (v9→v12), freeze→close-connections handler, in-memory LRU caches, age-based cleanup, batched writes | Mostly solid — migration chain, freeze-handler, and per-instance LRU caches are sound; `clearDatabaseForInstance`'s partial `clearCache` calls are redundant (the later `deleteDatabase` → `clearAllCaches` covers them) but harmless. **Fixed:** **D1** `getDatabase` now caches the in-flight *promise* instead of the resolved DB — previously concurrent first-time callers (the cold-load norm) each opened and then leaked a separate IDB connection, which could also block `deleteDatabase` on logout; failed opens are evicted so the instance isn't permanently bricked. Removed unconditional debug `console.log`s from the freeze handler (misplaced inside the per-instance loop), `cleanup`/`scheduledCleanup`, and `insertTimelineItems` (fired on every insert). **Noted (not fixed):** `deleteDatabase`'s `onblocked` only logs and never settles, so a *foreign* open connection (another tab) can still hang the logout/clear promise — D1 removes the common self-inflicted case. ✅ |
| 2026-06-14 | **Account-management features + log-driven fixes (1.9.0–1.9.2)** — reviewed during build and validated by the user's live testing (incl. against GoToSocial `social.ztfr.eu`) | follow/unfollow button + "⋯" on the Follows **and** Followers lists, `remove_from_followers`, graded empty-state + "Showing X of Y" partial notice, scroll-to-top, and the unfollow/block/lists **cache purge** (`removeAccountFrom{FollowGated,All}Timelines`, in-memory + IDB `deleteTimelineItemsForAccount`); plus poll rendering and action error-logging | Behaviour validated end-to-end. **Fixed:** poll `own_votes`/`options` crash that blanked the timeline (found via a source-map-resolved error log → `StatusPoll`); the list-purge fresh-session gap — refactored the IDB purge to a whole-store `home`/`list/` **key filter** (`isHomeOrListKey`, `{ homeAndListsOnly }`), so it no longer depends on the non-persisted `instanceLists`; transient network failures in follow/block/remove now log **warn** not error, and the `refreshAccountsList` handler `.catch`es (no uncaught rejection). **Confirmed server-side (not a bug):** stale-looking list entries after refollow + re-add come from GtS **live-querying** the list (verified in the `GET /timelines/list/:id` response JSON), not our cache — the purge is correct and surgical (pointers only, bodies + other accounts' cache preserved). **Capability gating (no Fediverse flag → can't pre-detect):** `remove_from_followers` uses show-then-toast; the Lists feature uses runtime 403/404/501 detection (`instanceListsSupported`). Both documented in §21. ✅ |
| 2026-06-14 | **Remaining lower-criticality areas + the two carry-overs** (clearing the §23 "revisit candidates" backlog) | (1) compose autosuggest — `autosuggest{Account,Hashtag,Emoji}Search.js`, `autosuggestObservers.js`, `RequestThrottler`; (2) the carry-overs — `getNotification` by-reference cache, `deleteDatabase` `onblocked`; (3) status-action plumbing — `react.js`, `favorite.js`, `deleteStatuses.js` | Autosuggest is sound: each keystroke cancels the previous search (`RequestThrottler` rejects in-flight **and** queued requests after `cancel()`; expected cancellations are swallowed, only genuine errors `warn`) and the app-level `observe` doesn't leak. **Fixed:** the **hashtag** search lacked the post-resolve `canceled` guard the account/emoji searches already have — a request that resolved in the tiny window just before `cancel()` could write stale results over a newer search; added the flag. **Carry-overs resolved:** `getNotification` now `cloneDeep`s on cache read+write (matches `getStatus`; stops callers/rehydration mutating the shared cached object); `deleteDatabase`'s `onblocked` now resolves best-effort (warn) so a foreign open connection (another tab) no longer hangs the logout/clear promise forever (the actual delete still completes in the background once that tab closes). **Status actions:** removed leftover debug `console.log`s in `deleteStatuses.js`; `favorite.js` (optimistic update + undo on failure) and `react.js` are otherwise correct. **Action-error logging sweep (done in a follow-up):** added a shared `logActionError(label, err)` (`_utils/isNetworkError.js`) — warns on transient network noise, red-errors genuine bugs — and routed every user-action catch that surfaces a toast through it (~20 `_actions/*` files: favorite, react, reblog, bookmark, pin, delete, mute, muteConversation, polls, requests, media, search, setShowReblogs, setAccountNotified, setDomainBlocked, compose, translate, addInstance, the remote-fetch paths in accounts, and follow/block converted from their inline form). Genuine non-network catches are deliberately left as `console.error` (DB reads in `accounts.js`, blurhash-worker start, `processFreshUpdates`, clipboard fallback); `timeline.js` already had the inline equivalent. **Hardcoded user-facing strings** (the remote-reaction refusal toast in `react.js`, the instance-login errors in `addInstance.js`) are internationalised in a follow-up — see the i18n row below. ✅ |
| 2026-06-14 | **Virtual-list follow-up** — the two items left "noted" by the 2026-06-13 virtual-list pass | (1) `VirtualListItem` `requestPostAnimationFrame` height write after destroy; (2) per-realm `itemHeights` unbounded growth | **Fixed (1):** the oncreate rPAF (which can't be cancelled) could still fire after a fast scroll destroyed the item; the old guard only checked `!node`, but a destroyed item's node stays truthy while detached, so `getBoundingClientRect()` returned 0 and wrote `itemHeights[id] = 0` — corrupting the offset sum and hiding the item on re-entry until re-measured. Added a `node.isConnected` check so the stale write is skipped. **Assessed, deliberately not changed (2):** `itemHeights` growth is bounded in practice — the realm LRU (`maxSize 10`) evicts whole realms (incl. their `itemHeights`) and logout/instance-switch clears by prefix; within a realm it grows in lockstep with `items` while each entry is just `id → number` (negligible beside the summary objects in `items`). A prune would have to run on every `items` change (the streaming hot path) at O(n) for no meaningful memory win — net negative, so left as-is. ✅ |
| 2026-06-14 | **i18n polish** — the last hardcoded user-facing English strings flagged by earlier passes | `react.js` remote-reaction refusal toast; `addInstance.js` login errors (already-logged-in, offline, invalid-OAuth-state, generic-instance, connect-failed); `media.js` "too many attachments" | **Fixed:** replaced every hardcoded string with intl keys and added them to all five locale files (`en-US`, `de`, `es`, `fr`, `ru-RU`). Followed the project's loader rule (`webpack/svelte-intl-loader.js` rewrites any `'intl.x'` literal via `getIntl` → `format-message-parse`): keys **without** a placeholder resolve to a plain string and are used as bare `'intl.x'` literals (incl. inside the login-error ternary and `createKnownError`); keys **with** `{instance}`/`{error}`/`{max}` resolve to an AST and go through `formatIntl(...)`. Parameterised values are kept apostrophe-free so `format-message-parse` doesn't mis-quote the placeholder; `instanceGenericError` keeps its `<a>` admin-guide link (a no-placeholder key, rendered via the existing `{@html}`). Verified all 35 entries for balanced braces, present placeholders, and no apostrophe-in-param. **Russian wired up:** renamed the orphaned `ru-RU.JS` → `ru-RU.js` (matching the other four) and registered it in `bin/getIntl.js`'s `locales` map, so `LOCALE=ru-RU pnpm build` now compiles the Russian strings. Two caveats, both graceful (no build break): the emoji-picker UI labels fall back to English (`emojiPickerLocales` in `webpack/client.config.js` only imports `fr`/`de`/`es` — left untouched, since importing a non-existent `emoji-picker-element/i18n/ru.js` would break *every* build), and a full ru build also needs `emoji-picker-element-data` to ship `ru` data (it does via CLDR; build-assets throws clearly if not). ✅ |
| 2026-06-15 | **Runtime i18n (v1.10.0)** — full review of the live-language-switch migration and the build-time `LOCALE` removal | resolver (`_intl/runtime.js`), registry (`locales.js`), loader (`svelte-intl-loader.js`), reactive `messages` computed, the script-side `$messages` reactivity sweep (~30 components + `_static/*` configs), the `LOCALE` removal (webpack configs, `bin/*`, `template.html`, emoji picker), and all four locale files | Engine sound: the fallback chain (locale → en-US → key) plus the `astToPlainString` empty-AST guard make a blank / "undefined" / `interpret()`-crash impossible; the loader correctly splits template (`$messages`) vs. script (`getMessage`) vs. `formatIntl` (key kept) vs. SW/`bin` (`buildTimeIntl`, English), and protects `formatIntl(` keys via a NUL marker; `messages` + per-locale `Intl.*` formatters are cached. **Fixed during the build-up:** placeholder messages (`followLabel`/`unfollowLabel` — plain in en-US but a `{requested}` select in de/es/fr/ru) were read via `$messages` and rendered **"undefined"** in non-English (follow + hashtag buttons) → moved to `formatIntl`; `VirtualList` (bound to `virtualListStore`, no `$messages`) crashed on an empty list → resolves "nothing to show" via `getMessage`; the settings breadcrumb and many persistent label computeds (page titles, status header/toolbar, follow buttons, list/empty states, settings labels, `_static/*` dropdowns) were **frozen at the boot language** → `$messages`-with-dep sweep; status-header action verbs split into present-tense header-only keys (`boostsAction`/`repliesTo`) so they read naturally without breaking the boost-confirmation announcement; the pinned-posts empty-state branch was missing → added. **Audited (scripts):** every referenced `intl.*` key (588) exists in en-US (the 4 "misses" are comment/regex examples); **0** unsafe `$messages` keys (plain-in-en-US-but-placeholder-elsewhere) remain; all four locales **100 % complete** (0 missing keys), hotkey help refreshed (`t`/`a` + `g+b`/`g+i`) with several stale/typo'd French/Spanish hotkeys corrected and Russian plural forms (one/few/many/other) used; locale files brace-balanced with no broken quoting (French ’ typographic apostrophes, no stray straight quotes). **Deliberate, documented (§15/§21):** emoji picker stays English (separate web component), and dialog-`data` / click-handler / foreign-store bare `'intl.x'` are kept (resolve at mount/action time, not frozen). Removed the now-unused `rtl-detect` dep. ✅ |
| 2026-06-15 | **Post-i18n hardening + locale completeness + runtime-config audit** — user-reported polish after the v1.10.0 build | the four locale files (de/es/fr/ru → 100 %), remaining hardcoded-English compose strings, the login language picker layout, the runtime env-injection path, and a debug-log sweep | **Locales completed:** all of de/es/fr/ru brought to **0 missing keys** (de 78 / es 110 / fr 174 / ru 152), stale hotkey help refreshed everywhere, French/Spanish hotkey typos fixed, Russian plurals (one/few/many/other); German "Tröt"→"Beitrag" terminology pass (post button → "Post"). **Compose i18n:** the edit/reply/quote headers, the don't-edit/reply/quote buttons and the poll "Add choice"/"Choice N" labels were **hardcoded English** (never intl keys — so untouched by the completeness pass) → added `editing/replyingTo/quoting/aPost/aPostBy/dontEdit/dontReply/dontQuote/addPollChoice` in all five locales (fr/ru use infinitive forms to avoid elision/case issues). **UI:** the login language picker used `min-height:85vh` + `margin-top:auto`, stretching the card to full height and forcing a needless mobile scroll → made it `position:fixed` (out of flow, bottom-centred, no stretch). **Runtime-config audit (the trigger):** confirmed every *current* deploy setting is runtime-injectable without a rebuild — `SINGLE_INSTANCE` via `entrypoint.sh` → `config.js` window var, `TRANSLATE_API` via `entrypoint.sh` `sed` into the nginx proxy; documented as §4 *Runtime configuration*. `LOCALE` was the lone build-time exception (now removed). **Cleanup:** removed 62 inherited debug `console.log`s across 30 files (incl. two logging user content — `updateStatus`, `showComposeDialog`), keeping only the server-startup log; verified no braceless-body danglers. Each step audited with the locale parser (0 missing, 0 unsafe `$messages`, balanced quoting/braces). ✅ |
| 2026-06-16 | **`ComposeAutosuggest` teardown crash** — log-reported after the dialog scroll-lock work | the autosuggest position-recalc path (`setup`'s rAF→rPAF chain, `calculateInsetInlineStartAndTop`, `doResize`) | **Fixed:** `calculateInsetInlineStartAndTop` did `this.refs.anchor.getBoundingClientRect()`, but `anchor` is `null` when the **uncancellable** `requestPostAnimationFrame` setup callback fires after the compose box/dialog was torn down (the dialog-timing change just made the latent race surface). Bail out of the rPAF setup when `refs.anchor` is gone, and return a no-op position from `calculateInsetInlineStartAndTop` for the async resize/scroll callers. **Pattern (now seen twice — cf. the 2026-06-14 VirtualList `requestPostAnimationFrame`-after-destroy fix):** an uncancellable `requestAnimationFrame`/`requestPostAnimationFrame` callback **must** re-check refs / mounted state before touching the DOM, since (unlike observers/listeners) it can't be cancelled on teardown. ✅ |
| 2026-06-16 | **rAF/rPAF-teardown sweep + new scroll-lock lifecycle** — applying the rule above across all `requestAnimationFrame`/`requestPostAnimationFrame` callers, and reviewing the just-added dialog scroll-lock | ~30 rAF/rPAF callers (focus on those reading `this.refs`/`getBoundingClientRect`) + `ModalDialog` `setPageScrollLock` lifecycle | Most callers are already safe — either cancelled on destroy (`ScrollToTopButton` stores `this._raf` + `cancelAnimationFrame` in `ondestroy` — the model pattern) or guarded (`VirtualListItem` `isConnected`). **Fixed — scroll-lock leak (regression from the v1.10.1 `<html>` lock):** `setPageScrollLock(false)` ran only in a11yDialog's `'hide'` handler, so a teardown that bypasses `close()` (e.g. route navigation with a dialog open → `ondestroy` → `a11yDialog.destroy()`, no `'hide'`) left the ref-counter incremented → **page permanently frozen**. (The old `body.modal-open` lock had the same gap but was a no-op, so it never showed.) Release is now driven by a per-instance `_scrollLocked` flag from **both** `'hide'` and `ondestroy`, exactly once per lock. **Hardened (latent null-crashes on fast teardown):** added `!this.refs.node` / `!this.refs.textarea` guards to the uncancelled rAFs in `VirtualListHeader` (header height measure) and `ComposeInput` (WebKit reply caret). **Accepted (low):** `ModalDialog`'s show-path rAF reads `this.refs.backdrop/node.animate()` without a null-check, but it runs on a just-created dialog (teardown inside the setTimeout→rAF window is negligible). **Also swept** `setTimeout`/`scheduleIdleTask` callbacks (0 unguarded ref reads — that class is clean) and **verified the list/tag 5xx retry**: loop-safe (exactly one retry per fetch — the retry call isn't re-wrapped — then graceful fallback), and 4xx are correctly excluded because `ajax.js` sets `err.status` on every ≥300 response, so `!e.status || e.status >= 500` only matches 5xx + status-less network errors. ✅ |
| 2026-06-20 | **[v1.10.6] Virtual list blank-until-scroll on cold open** — user-reported: a freshly-opened timeline (most visibly a brand-new/empty list) showed nothing — not even the loading footer or empty state — until a manual scroll/resize. Confirmed a render/measure bug, not a fetch bug (the open-fetch is requested in `Timeline` `oncreate`; the list stayed blank even while waiting without scrolling) | `VirtualListContainer` viewport/scroll measurement lifecycle (`onResize`, `onScroll`, the `scrollTop>0` vs `===0` mount branches), `virtualListStore` `rawVisibleItems` (render buffer = `RENDER_BUFFER_FACTOR * offsetHeight`) | **Root cause:** the viewport/scroll metrics were measured **only synchronously at mount** — and `offsetHeight` only in the `scrollTop===0` branch — before the just-navigated route's list/footer were laid out, and nothing re-measured them afterwards. Stale metrics (notably `offsetHeight` 0 → render buffer `4*0=0`) collapse the `rawVisibleItems` window to nothing, so the list renders empty until a scroll/resize re-reads the metrics. Cache-backed timelines (home/federated) hid it because their cached items make the mount-time layout already correct; cold/uncached lists exposed it. **Fixed:** (1) call `onResize()` unconditionally at mount (was missing in the `scrollTop>0` restore branch); (2) re-measure once after layout settles (`doubleRAF`), **unconditionally** — this is what fixes the empty/fresh-list case where no items ever arrive to trigger a recompute; (3) re-measure again the first time items arrive (a slow fetch can resolve after the initial settle), via observing the **component** computed `length` (mirrors `$length`) so Svelte auto-removes the listener — no store-listener leak. The deferred re-measures are uncancellable `doubleRAF`s guarded by a `_destroyed` flag (rAF-after-teardown rule, cf. the 2026-06-14/16 rows), so a run after teardown can't write a stale realm. ✅ |
| 2026-06-20 | **[v1.10.5–1.10.7] Exclusive-lists / Manage-lists feature + the v1.10.4/v1.10.6 timeline fixes** — full review requested after the feature landed | API (`_api/lists.js`), actions (`_actions/lists.js`), exclusive→home purge (`_actions/timeline.js`, `_database/timelines/deletion.js`), capability detection + store (`instanceListsExclusiveSupported`/computed), UI (`CreateListDialog`, `ManageListsSettings`, `CommunitySettingsSection`, `ListMembershipDialog`), and the two timeline fixes (`fillStreamingGap`, `VirtualListContainer`) | Engine sound: capability via `exclusive`-key presence (no allowlist, optimistic-when-unknown, create-dialog read-back toast) is consistent with the §21 [v1.9.1] pattern; the home purge correctly mirrors unfollow semantics (boost `ACCOUNT_ID`, pointers-only, home-only so members stay in the list) and the reverse paths defer to the server via mark-stale (§21 [v1.9.2]); membership edits are centralised through `addAccountToListAndPurge`/`removeAccountFromListAndRestore`; no import cycle (`lists.js → timeline.js` one-way); all `formatIntl` uses carry placeholders, plain keys via `$messages`/`getMessage`, locales balanced. **Fixed during review:** (1) **perf** — turning a list exclusive purged home with **one full IDB `status_timelines` cursor scan per member** (O(members × store)); added `deleteTimelineItemsForAccounts` (Set-based, **single** scan) + `removeAccountsFromHomeTimeline`, so `setListExclusive` now batches; the singular forms delegate to the plural; (2) **double-fire** — the exclusive toggle had no in-flight guard (turning on takes seconds: member fetch + purge), so rapid clicks could fire several PUT+scan runs → added a `togglingListId` guard + `disabled` on the checkbox, reverting a concurrent flip. **Accepted (low):** `getListAccounts` uses `limit=0` (all members, no pagination) — if a backend ignored it only the first page would purge (rare, degrades gracefully); turning exclusive on for an unsupported backend silently no-ops home (the toggle then hides via the support flag — no toast, unlike the create dialog, deemed sufficient). Timeline fixes re-confirmed: `fillStreamingGap` try/catch + slow-timeline cap (2) + smaller batch is loop-safe; `VirtualListContainer` re-measure is teardown-guarded. **Verified on dev (Mastodon + GoToSocial):** exclusive toggle purges members from home; non-exclusive / member-removal / list-deletion restores them on the next refresh. ✅ |
| 2026-06-20 | **[v1.10.8] Removing an account from a list left its posts in that list's feed** — user-reported regression: deselecting a list in the profile "⋯" → manage-list-membership dialog removed the membership server-side, but the account's posts stayed in the list timeline | the list-membership removal path (`ListMembershipDialog` → `removeAccountFromListAndRestore`), list-timeline cache | **Root cause (long-standing, since v1.8.4):** removing from a list never purged the list's cached timeline — it only called the API. Before list timelines became **cache-first** (v1.8.4), this was masked because a cold load fetched the list fresh (server-filtered); once cache-first reached lists, the IDB cache prefilled the removed account's posts and the union-only merge kept them, so they lingered until age-cleanup. The recent exclusive-lists work routed removal through an action but didn't change this (it only handled home). **Fixed:** `removeAccountFromListAndRestore` now purges the account from **that one list's** timeline — added `removeAccountFromSingleTimeline` (in-memory filter + IDB pointers) backed by a new `{ timelineName }` scope on `deleteTimelineItemsForAccounts` (matches `list/<id>\u0000…` keys only). Mirrors the unfollow/block purge model (§21 [v1.9.2]) scoped to a single list; in-memory purge updates the view live if you're on the list, IDB purge covers the next visit. **Final pre-release review (1.11.0):** the list-prefix scope (`list/<id>` + NUL) can't collide across ids (the NUL delimiter stops `list/12` matching `list/123`); removing from an **exclusive** list correctly does both — purge the account from the list feed **and** `markHomeStale` so it returns to home; `ACCOUNT_ID` boost semantics keep other members' boosts of the removed account; no import cycle (`lists.js → timeline.js` one-way); NUL-byte- and footgun-free. Clean. ✅ |
| 2026-06-21 | **[v1.11.1] Post-release latency pass** — acting on an external performance review of the timeline/list read path | the cold-list timeout strategy (`fetchTimelineItemsFromNetworkWithRetry`, `getTimeline` timeout selection, `ajax.js`), and an assessment of the per-stream-item re-filter cost (`filteredTimelineItemSummaries`, `createFilterFunction`) | **Fixed (#1 — worst-case cold-list latency):** a hanging cold list/tag could block ~80 s (first attempt **and** retry each used the full 40 s `SLOW_READ_TIMEOUT`). The first cold attempt now fails fast at `SLOW_READ_TIMEOUT_FIRST` (= `DEFAULT_TIMEOUT`, 20 s) via a `timeoutOverride` threaded through `getTimeline`; only the single retry keeps the 40 s headroom (backend warmed by then). Worst case ~60 s; the common GtS warm-up case is unchanged or better, and a steady-slow server still gets 40 s on retry. **Assessed, deliberately deferred (#4 — O(n²) re-filter over a session):** `filteredTimelineItemSummaries` rebuilds via `.filter()` on every streamed item. But the predicate (`createFilterFunction`) is **trivial** — property checks + an `includes` on the pre-computed `filterContexts` (word-filtering is stamped in at summary-build time, not at filter time) — so a per-id predicate memoisation would save nothing, and each individual recompute is sub-ms until thousands of items. The only real fix is an invasive incremental-array rewrite of the central display/bundling flow; not worth the risk right after a release for a long-session-only gain → stays backlogged. **Verified clean:** retry path unchanged (no override → name-based 40 s); non-slow timelines unaffected (no retry, `DEFAULT_TIMEOUT`); `fillStreamingGap`/`fetchPagedItems` paths unchanged. ✅ |
...
| 2026-09-26 | **Production 1.12.0 console-log triage** | source-map resolution via the live prod chunks (`_database/timelines/{cacheStatus,updateStatus,getStatusOrNotification}.js`, `emoji-picker-element/database.js` + `picker.js`), `_utils/emojiDatabase.js`, `EmojiDialog.html`, `Status.html` reply-header lookup, `_actions/stream/processMessage.js` | **Real fix (1), still present in 1.12.0 and dev:** `⛔ reading 'id'` at `cacheStatus` ← `doUpdateStatus`. `getStatus` cached a miss as `undefined`, so `hasInCache` was true while `getInCache` returned `undefined`. Every `status.update` for that id crashed, and favourite/boost/bookmark on it threw after the API call. The cache now holds found records only and never overwrites an entry written during the read; `doUpdateStatus` tests the value (§7). Verified in Node with the real modules and a mocked IDB: HEAD reproduces both errors, the fix passes all six cases. **Real fix (2):** `⛔ Failed to fetch: /emoji-en-US.json: 502` came from an unguarded `_lazyUpdate`. The logged session most likely predates 1.12.0 (see below), but the picker's internal `Database` still had the same gap in 1.12.0 and is now guarded (§10). Verified with a mocked picker instance. **Benign/expected:** `timeline fetch failed: home · Failed to fetch`, `background refresh failed`, `streaming gap fill failed · 500` and the 502 on the static emoji file all point to an outage of the server/host, not a code path. **Log-age caveat:** `background refresh failed` was printed with a full stack, which is the 1.11.0 format (since 1.11.5 only the message is logged), and the 1481/6389 chunk hashes no longer exist in prod. Persisted logs survive an update and sit under the new banner (§20). |
| 2026-09-26 | **General hardening review (1.12.3 → 1.12.4)** — whole codebase, focus on the neuralgic areas; six parallel read-only passes (quotes/filters incl. the unreviewed 1.12.1 code, timeline/streaming, DB/caches, status/notification/profile rendering, push/SW/auth/deploy, compose/actions/dialogs), each finding re-verified against the code before fixing | all of `_database/**`, `_actions/**`, `_api/**`, `_thirdparty/websocket`, `_utils/{renderPostHTML,resolveCardForUrl,ajax,sync,quotes}`, status/compose/dialog/profile components, `service-worker.js`, `docker/*`, `.gitea/workflows/*` | **Security:** removed `doQuickLoginIfNecessary` (link-driven session swap, bypassed the OAuth `state` check); `renderPostHTML` drops active/embedding elements, `on*` attributes and `javascript:`/`vbscript:`/`file:`/non-image `data:` URLs (the CSP already blocked script, so this is defence in depth); escaped reaction names and login error text; nginx `X-Frame-Options`/`frame-ancestors`/`nosniff`/`no-referrer`; `flavor: latest=false`; `nginx:stable-alpine`. **Rendering:** `::` was dropped whenever a post had no custom emoji (`:():` matched empty); `walkElements` iterated a live array (second emoji double-wrapped, adjacent `RE:` lines half-stripped); profile page crashes on `header: null` / bad `created_at`; translated long/filtered posts showed "undefined"; external links now open after ≤ 1 s instead of waiting up to 20 s for the in-app resolve (popup blocker). **Data integrity:** `dbPromise` `onabort`; flag setters skip unstored statuses (favouriting a search result threw); unrenderable items dropped in `createMakeProps`; failed IDB writes no longer swallow posts or undo accepted actions; `onversionchange`; logged-out databases not re-opened (401 path via `sync.js`, in-flight fetches); cleanup `maxWait`; `getReblogsForStatus` returned nothing; `LocalStorageStore.save` quota-safe; double logout dropped the *last* account. **Timeline/streaming:** stream keyed on the instance; one WebSocket at a time (reconnect/backoff leaks); thread filter collected every reply when its status was missing; deletes missed `notifications/mentions`; gap-fill mentions; favourites/bookmarks fresh-vs-paging + error state; streamed edits shown live; `quoteHidden`/`filterContexts` refreshed. **Quotes/filters:** warn filters matched processed content (missed trailing hashtags / emoji — the 1.12.1 bumblebee case); withheld quotes no longer become cards; redraft keeps Mastodon quotes; card cache per instance, in-flight shared, failures retried. **Compose/profile:** GtS profile fields, post-while-uploading, draft replacement confirm, Mastodon edit `media_attributes` (+ existing focal points), media processing wait, server error reasons, poll option limit, file input reset, reply focus (mitt passes one payload), share-cancel toast, edit error toast, options menu no longer hijacks the viewed profile (own relationship load; id-guarded store writes). **Push/auth/deploy:** browser unsubscribe on logging out the push account, VAPID key check on reused subscriptions, dummy subscription cleaned up, self-heal respects the single-account model, per-instance record in sync, OAuth callback errors shown, timeouts on boost/report/push calls, nginx cache split, entrypoint validation. **Verified:** Node suites with the real modules and mocks (IDB, WebSocket, fetch, store), each checked to fail on the old code where applicable; 18 changed components compile (svelte 2.16.1); `standard` (JS + HTML) shows no new findings vs HEAD, `prettier` clean. **Not verified:** a real browser, and `nginx -t` on the new config (no nginx/docker here) — check on dev. **Open:** see revisit candidates below. |
| 2026-09-26 | **Several tabs/windows (1.12.4 → 1.12.5)** — never reviewed before | `LocalStorageStore`, `store.js` persisted keys, logout/login flows, push entry points, `showDesktopNotification.js`, `service-worker.js` push dedup, IndexedDB lifecycle across tabs, compose drafts | **Root cause:** localStorage was read once per tab, and `save()` wrote the tab's load-time copy of every key it changed. **Fixed:** `storage` events taken over (not written back; bfcache re-read), `currentInstance` per tab, per-draft merge of `composeData` (`ts` vs. last sync), a logout elsewhere marks the database logged out and reloads a tab on that account, push under a cross-tab Web Lock with the state re-read first plus endpoint self-heal, the sound once per notification, account-aware push suppression in the service worker. **Fine:** OAuth logins in two tabs (state check), IndexedDB inserts, the 60 s poll/cleanup (active page only), SW updates (two builds cached). Tested: store sync incl. the draft merge. |
| 2026-09-26 | **Long-running tab/PWA (1.12.4 → 1.12.5)** — never reviewed before | timeline data and buffers, `isEqual` hot paths, per-status maps, module caches, listeners/timers, log persistence, IndexedDB growth | **Fixed:** streamed timelines and their buffers had no bound (thousands of posts per hour on local/federated; each insert O(n) plus whole-array JSON compares, measured ~17 ms at 20k items): capped at 500 when merged at the top, buffer overflow restarts the list without a gap, `arraysEqual`/length compares; data of all visited timelines kept until logout → 30 most recent kept (+ pinned posts); stored log entries capped at 4000 chars and three whole-object logs trimmed. **Bounded already:** persisted state (per-instance maps, drafts pruned after 5 days), per-status UI maps (grow only with user actions), all LRU caches, listeners/timers (removed on destroy / paused while inactive), IndexedDB (age cleanup at least every 30 min). **Note:** IndexedDB is bounded by age, not size — a busy federated tab stores 5 days of posts. Tested: caps and LRU. |
| 2026-09-26 | **Test coverage (1.12.5 → 1.12.6)** — tests for the remaining neuralgic areas, each checked to fail when the code it guards is broken | `src/intl/*`, word filters, `statusIdSorting`/`sortItemSummaries`, `_actions/pushSubscription`, `service-worker.js` push handler, `_actions/addInstance` (OAuth), `processContent/{hashtagBar,mfm}`, `libreTranslate`/`_actions/translate`, `_database/**` on `fake-indexeddb`, `bin/run-tests.js` | **Found:** es/fr/ru `pageTitle` hard-coded "Zocial" instead of `{appName}`; direct replies of the focused status kept their input order (deeper levels were sorted). **Fixed** both. **Runner:** `--test-timeout=30000`, `--test-force-exit` and a 10-minute cap, because a database delete blocked by a connection that never closes kept the test process alive, so a regression there would have hung the Docker build instead of failing it. **Mutation-checked:** removing `filter(Boolean)` in `getTimeline`, `onversionchange`, the `onblocked` resolve, the snowflake migration or cleanup batching each fails a test. **Not covered:** `libreTranslateHTML` (needs a DOM) and Svelte components. |
...
**Safety net since v1.12.5:** fixes in these areas come with a test in `test/`; the image build refuses to run with a failing test or lint error (§4).

**Component logic goes into tested functions (2026-09-26).** Svelte components get no tests of their own, on purpose: most component fixes were layout, timing and mobile issues (virtual-list measuring, scroll lock, focus, positioning) that a DOM without layout in Node can't check, and testing Svelte 2 components would need its own compile step, the intl rewrite, a DOM and the store. The logic bugs (the profile crash on `header: null`, "undefined" above translated posts) sat in computeds. So when a component breaks because of its logic, the fix moves that logic into a plain function under `_utils` (or next to the action) and tests it there; the component only calls it. Layout and device behaviour stay a matter of testing on dev.
...
- **Test for `libreTranslateHTML.js` (translation HTML round trip)** — only when it becomes relevant: when that file changes, or the translation backend is switched or upgraded. It needs a DOM in Node (happy-dom or jsdom as a second `devDependency`), which isn't worth it for code whose logic hasn't changed since the fork (only a rename and passing on the detection confidence). Cover then: links, mentions and formatting keep their place after translation (the `<a i=N>` markers, including the fix-up for spaces the backend inserts, `< a` / `< /a>`), the content warning is split off, entities survive, and a backend answer with script or event handlers comes out sanitised. A stubbed backend can't tell whether a new LibreTranslate still keeps the markers, so after a backend upgrade also translate a post with links on dev.
...
- ~~**Mentions tab leaks newer types (REST).**~~ **Fixed in v1.12.5:** the Mentions fetch sends `types[]=mention` and `include_types[]=mention`, keeps `exclude_types[]` as the fallback, and filters client-side. Original note: `notMentions` (`_static/notifications.ts`) is built from the keys we know, so `quote`, `quoted_update`, `severed_relationships`, `moderation_warning`, `added_to_collection` and `collection_update` are not excluded and appear in the Mentions tab on REST loads (the streaming path correctly adds only `mention`). Fix: request `types[]=mention` (Mastodon ≥ 3.5, **GtS ✓**, Pleroma unverified).
- ~~**`quote` / `quoted_update` render as "Unhandled notification type".**~~ **Fixed in v1.12.5**, together with `severed_relationships`, `moderation_warning` and `annual_report` (the last three as a plain sentence without avatar and name, `selfEvent`, since the server sends the user's own account). Original note: They are baseline types, so the server will never send a fallback for them. They need native `notificationInfos` entries.
- ~~**Profile fields can be lost on GtS**~~ **fixed in v1.12.4**: the dialog offers `max(existing fields, configuration.accounts.max_profile_fields or 4)` rows and never truncates. Original note: **Profile fields can be lost on GtS (code-read, verify on a GtS account).** `EditProfileDialog` hard-codes `FIELD_SLOTS = 4`, and `fields_attributes` replaces *all* fields. GtS allows 6, so saving would likely drop fields 5–6. Fix: read `configuration.accounts.max_profile_fields`.
