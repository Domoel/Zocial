...
**Tradeoffs:** (a) A repeated lookup of a post that isn't stored now costs one read-only IDB transaction each time (the reply-header lookup on every mount of such a reply). That cost is small and async, and the alternative (a negative cache) goes stale as soon as the post arrives by a path that doesn't call `cacheStatus`, such as the IDB branch of `doUpdateStatus`. (b) The account/relationship caches (`getGenericEntityWithId`) still cache misses. Their callers only read and fall back to the network, so a stale miss costs a refetch, not a crash; left as is to keep the change narrow.
