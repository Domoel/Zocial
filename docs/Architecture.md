...
- **A few hardcoded user-facing strings** still want intl keys (e.g. the remote-reaction refusal toast in `react.js`, the generic instance-login errors in `addInstance.js`) — i18n polish, not a correctness issue.
- **API/ajax endpoint modules** (`_api/*`): thin wrappers, stable — only revisit if a specific endpoint misbehaves.

The shared action-error logging sweep (`logActionError`) is done — see the 2026-06-14 row above.
