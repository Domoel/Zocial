...
| **1.11.0** | 2026-06-20 | **Production release** rolling up the 1.10.1–1.10.7 dev line. Two features — **exclusive lists** + a *Manage lists* page (rename/delete/exclusive toggle), and **first-visit language detection** (follow the browser, fall back to English) — plus timeline-reliability and rendering fixes: streaming gap-fill hardened for slow Mastodon/GtS lists, list/tag 5xx cold-load retry, timelines render immediately on open, mobile compose scroll-lock on `<html>`, no layout shift on dialog open (`scrollbar-gutter`). Full notes: [`docs/release-notes/1.11.0.md`](release-notes/1.11.0.md) |
