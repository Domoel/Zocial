...
| 2026-06-13 | **Virtual-list internals** (`_components/virtualList/*`, `RealmStore`, `reselect`) | height measurement (batch + on-demand recalc), `rawVisibleItems` windowing maths, `reselect` re-render guard, realm LRU cache, scroll restoration, freeze/fullscreen handling, Svelte-2 store-observer leak hack | Solid, mature Pinafore code — no correctness bugs. The store-observer leak is already worked around (component-mirror computeds), in-place `itemHeights` mutation is fine (Svelte-2 `set` recomputes by provided key, not identity), and the `height:0` collapsed-bundle case is deliberately handled (`=== undefined`). **Fixed:** removed leftover debug `console.log`s (incl. one firing on every scroll-restore height-completion and one on every fullscreen toggle); `teardownScroll()` now `.cancel()`s the throttled scroll listener so a trailing call can't write `scrollTop`/`scrollHeight` into a null realm after `ondestroy`. **Noted (not fixed — needs runtime profiling):** per-realm `itemHeights` grows unbounded over a long-lived timeline (never pruned), and a `requestPostAnimationFrame` height write can still fire for an item destroyed before it runs. ✅ |
