...
- **Unblock does NOT auto-restore.** Block *deleted* the cached posts (in-memory + IDB pointers), so nothing is restored from local cache — content only comes back from the server, and the two paths differ:
  ...
  We deliberately do **not** auto-refresh on unblock: the gain is small (unblock is rare) and a forced refresh would surface via the standard path anyway (direct at the top, "Show X more" when scrolled — never a mid-scroll jump). Reload/navigation brings the current window back.
